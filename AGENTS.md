...
- 当前基线 = **v3.0.0 已全渠道发行**（2026-09-18；回执见 RELEASE-CHECKLIST I 节与项目信息 §五）；**v3.1.0 = 发行前准备态（未发行）**——判据可信度批（判据版本化 j1.0→j2.3 / 金样本回归 20/20 / scorecard 指纹 / 形态分型 / 聚合器 / 判据史）+ 细则 344→368·29 类（v3.0 批① + v3.1 批，编号至 #368）+ 门禁 **8 项** + 无头全矩阵 20/20 + v3.1.0 部署后实跑 24 针 22 PASS；发行批动作见 RELEASE-CHECKLIST J 节，跑前确认注入副本已重部署且新会话读到「在场提示 · v3.1.0」（注入版本=会话创建时快照，部署后须重启应用）。
