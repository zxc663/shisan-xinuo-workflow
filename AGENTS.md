...
- 当前基线 = **v3.2.0 已全渠道发行**（2026-09-20；回执见 RELEASE-CHECKLIST L 节与项目信息 §五）——外部评审审计 + 条款级落刀批（细则 **373 条·30 类**，编号至 `#374`：清单外高危域枚举 / `GATE ev=` 验证层级 / 记忆档双指标 / 机器事实优先 / 副本内容哈希）+ 判据 **j2.5**（金样本 **23/23**）+ 门禁 **8 项** + 四个机检端口（`risk_scan` / `agent_log_rotate` / `gate_audit --gate/--high-risk/--independent-cmd` / `deploy_injection --check --hash`）。**三面已校准**：源库 3.2.0 · 五平台注入 `--check` 5/5（count=373）+ `--check --hash` 5/5 `HASH-OK`（`sha256:2a64ca815ad4`）· 六处技能副本 6/6 = 3.2.0。新会话验收锚=「在场提示 · v3.2.0」+ `373 条细则` + `zxc663` 应答（注入版本=会话创建时快照，**GUI 长活会话须重启应用**）。
