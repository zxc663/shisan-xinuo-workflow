...
25 本仓库 = 十三希诺 Agent 工作流 Skill 的源库与标本合库。**本文件是项目级注入**（每会话自动进入），与平台全局硬注入（injection-core，负责通用纪律）互补：项目级管「本仓库特有信息 + 项目承载 + 维护纪律」。内容精简，细节按需读 docs/project-info.md（导航）。
...
- 改 Skill 产物后：**全仓 grep 承载点口径**（细则条数/类数/版本号/在场提示）→ 跑 `scripts/verify-release.ps1`（内容锚点/hooks/版本/泄漏/正文净化/索引/事实对账 7 项门禁）→ 同步各副本（`python scripts/syncer.py`，WorkBuddy 侧加 `--dest`）。
...
- 发行动作（npm / GitHub Release / Gitee / ClawHub / About）必须先获用户批准 + `verify-release` 7/7 PASS + 观测阶段。
- 当前基线 = v2.9.0 **已全渠道发行**（2026-09-16：GitHub Release / npm 2.9.0 / Gitee / ClawHub 1.0.17 / About 双端；2.9 修正批——独立审查 P1×11+P2×16 机制级 23 项+v11 四项裁决+G 清单四点立条 #333-#335+security 应用安全基线节 OWASP 2025+G 直写批 #336-#344+#295 双击转正+推荐序施工批 #345+思考链 hooks 行，细则 330→355·26 类）；跑前确认注入副本已重部署且新会话读到「在场提示 · v2.9.0」。
