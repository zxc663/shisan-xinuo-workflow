...
- 当前基线 = **v3.1.0 已全渠道发行**（2026-09-19；回执见 RELEASE-CHECKLIST I-1 节与项目信息 §五）——判据可信度批（判据版本化 j1.0→**j2.5** / 金样本回归 **23/23** / scorecard 指纹 / 形态分型 / 聚合器 / 判据史）+ 细则 **373 条·30 类**（编号至 `#374`）+ 门禁 **8 项** + 无头全矩阵 20/20 + v3.1.0 部署后实跑 24 针 22 PASS + 2026-09-20 独立审计新采样 7 针 5 PASS。**源库领先于已部署副本**：评审提案落刀批（#370-#374 + 四个机检端口 + j2.5）**尚未重部署**，故 `deploy_injection --check` 会因副本 count/哈希滞后而红——这是预期态，重部署后恢复；新会话验收锚=「在场提示 · v3.1.0」+ `zxc663` 应答（注入版本=会话创建时快照，部署后须重启应用）。
