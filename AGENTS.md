...
- 改 Skill 产物后：**全仓 grep 承载点口径**（细则条数/类数/版本号/在场提示）→ 跑 `scripts/verify-release.ps1`（内容锚点/hooks/版本/泄漏/正文净化/索引/事实对账/**判据自测** 8 项门禁）→ 同步各副本（`python scripts/syncer.py`，WorkBuddy 侧加 `--dest`）。
...
- 当前基线 = **v3.1.0 已全渠道发行**（2026-09-19；回执见 RELEASE-CHECKLIST I-1 节与项目信息 §五）——判据可信度批（判据版本化 j1.0→**j2.5** / 金样本回归 **23/23** / scorecard 指纹 / 形态分型 / 聚合器 / 判据史）+ 细则 **373 条·30 类**（编号至 `#374`）+ 门禁 **8 项** + 无头全矩阵 20/20 + v3.1.0 部署后实跑 24 针 22 PASS + 2026-09-20 独立审计新采样 7 针 5 PASS + 评审提案落刀批。**落刀批已重部署并验收**（2026-09-20 20:42）：五平台注入 `--check` 5/5（v3.1.0/count=373）+ `--check --hash` 5/5 `HASH-OK`（`sha256:2a64ca815ad4`）；技能副本三包双通道同步；新会话实弹 **2/2 PASS**（指纹 `carrier_zcode=8a74176e730a` / `core_md=c06cdb347545` / `judge=j2.5`，见 EVIDENCE §四十）。**未 bump 版本、未发行**；副本头注版本串仍为 v3.1.0（内容以哈希为判据）。新会话验收锚=「在场提示 · v3.1.0」+ `373 条细则` + `zxc663` 应答（注入版本=会话创建时快照，**GUI 长活会话须重启应用**）。
