...
>
> 本仓库实质上就是一个**规范工程化的大模型 Agent 提示词注入标范与标本合集**——你可以直接拿本 Skill 的内容当**工作流样本**来打磨你自己的 Agent 工程规范。版本路线：**v2.0.5 之后进入稳定的「细则类小更新」**（触达问题已明确为**提示词边界问题**），不再做破坏性的大版本改动；但**不做绝对保证**（可能仍有意外情况）。**一个必须直面的边界**：哪怕在触达上做了重重努力（注入三层 / 项目承载 / 委托纪律包），这终究是**提示词范畴**——抵不上平台级「让 Agent 新会话自动加载」这样一句机制保证。**验收判据（可执行）**：新会话常驻不可用，或自检彩蛋 `zxc663` 未触发 → 即证明本 Skill 需要**用户主动触发或配置触发器**（平台 hooks / SessionStart），别把「提示词在场」误当「机制在场」。**更新（2026-09-09）**：该判据的触发器路线已走通并经机评路测机器实证——hooks / SessionStart 注入→行为跟随、门控零误触发、并发不丢（见 ①c 与 EVIDENCE §十八/十九）；配置触发器后，机制级触达可闭环。感谢加星 ⭐。
>
> *At its core this repo is a specimen collection of disciplined, engineering-grade prompt-injection standards for LLM agents — use it as a sample workflow to polish your own. After v2.0.5 expect stable, fine-grained updates; no breaking releases planned (but no hard guarantee). One boundary: however much injection/carrier/delegation effort goes in, this is still prompt-domain — it cannot beat a platform guarantee like "auto-load every new session". Acceptance check: if a fresh session is never aware of it, or the `zxc663` self-check does not fire, this Skill needs explicit triggering or a configured trigger (hooks / SessionStart) — do not mistake "present as prompt" for "present as mechanism". Thanks for the star.*
...
> **核心价值（作者实证，2026-09-11 定稿）**：真正的价值是**会话过程中执行的纪律**——复述 / 判级 / GATE / 留档这些动作的真正产物，是一套**完整的开发日志**（`memory/agent-log.md` 四区 + GATE 可重跑块 + 决策留档 + 细则命中取证），并由此带来**多会话接力能力**：任何新会话读一屏状态段即可零污染接续、立刻知道自己该干什么（冷启动只是这条链路的瞬时表现，不是价值本源——**纪律是因，日志是产物，接力是能力**）。机器实证：记忆续接 2/2（EVIDENCE §十九 B4 / §二十 C2）。判分体系以「新会话初始化质量（NQ）」为第一指标（接力的接收端质量：首轮定向 / 记忆接续 / 零污染 / 定向成本 / 载荷正确性，v4 回溯基线见 docs/version-plan-v270.md）；而 NQ 的上游正是会话内纪律执行与日志完整性——没有过程纪律，接力无物可接。

> **护城河（管道 × 水，2026-09-11 补录）**：冷启动协议是**管道**，`details.md` 的教训库才是**水**——新会话恢复出来的不只是「我在哪」，还有「我上次为什么在这、试过什么、什么行不通、下一步往哪走」。市面状态方案存的是**事实**（项目用 Prisma / 上次做到登录页），本库存的是**带事故现场、证据数字、根因与预防动作的失败记录**：#294 Edit/脚本双通道混用占全部工具错误 55%（186/337）、#233 命名直觉假绿附四类反例、#256 异步栈丢调用点出自一次真实 issue 误判——每条带技术栈标签、来源注记与命中计数，经路测跨池验证，按双击晋升 / 防棘轮双向流动。三要素乘积里，状态恢复与治理机制可被模仿，**「真实踩坑、如实记录、公开失败」的积累无法被模仿**（壁垒是时间差与意愿，不是秘密）。lookup 检索端口就是这只水龙头（v2.7 召回修复后 24/24）。覆盖边界如实声明：教训主体沉淀自 Web 全栈，Node 开源库 / 终端 UI 为边缘场景。

**真实口径（2026-08-30 首测；2026-09-09 全量刷新，本地 db.sqlite 穷尽取证）**：

**① 工程消费与成本（平台聚合）**：细则层（现 313 条 / 17 类）**工程消费命中此前 = 0**（v1.13 全平台审计），v1.13-1.16 以「错误必经句 + 预读 TOP + 命中取证行」修复后转**有命中**（#228×3 / #229×2 / #233×1，sess_c0f4df2b 留痕）。成本实证（**2026-09-09 刷新**，2026-08-28→09-09 窗口）：ZCode 平台累计 **input tokens 2,526,616,450（25.27 亿）/ 9,737 次模型请求 / 108 会话**（output 737 万；与官方「应用用量」面板累计 25.3 亿**精确吻合**，本地与官方双口径交叉验证）——**好的**：108 会话**零 context_exceeded**；会话 input 集中度 top3 = 37.8%（对比 08-30 旧口径 81%——长会话病态集中已缓解，v2.x 上下文管理与留档机制启用同期）；**坏的**：单次请求峰值 input 932K、重试 356 次、单会话峰值 4.06 亿 input 仍未消除；工具调用 11,694 次 / 错误率 2.9%（**最高频坑 F8：Edit 通道状态冲突 186 次 = 全部错误 55%**——脚本批量与 Edit 工具混用所致，已立 #294 + TOP 内联；其余多为授权边界正常拦截）。**缓存语义（2026-09-09 二批补测）**：input 中 **98.2% 为 cache_read**（新鲜 input 仅 ~4,615 万 / 1.8%）——峰值 932K 实为 99.9% 缓存读，长会话重复上下文按缓存计价，成本叙事以「新鲜 token」为准（details 口径见 EVIDENCE §十四）。**注明**：25.27 亿是平台聚合，非 Skill 归因（归因明细见 EVIDENCE §九）；常驻开销实测 ~3-4.5%（~6-9K tok / 200K，2026-09-07 实测）；近 7 天（09-03→09-09）增量 input 13.18 亿 / 5,757 请求。
...
**①c 机评路测（全自动无头驱动，2026-09-09/11；四场 37 会话，含 1 例探索性补谱系场景，机器信号预注册判分；每场景 N=1 不报泛化率）**：**v1 修复发现轮（10 会话）**：复述/判级/GATE 8/8，承载静默跳过 1 违规，detail_lookup 端口结构性不可达（部署面缺陷），歧义必问 0/1。**v2 修复效力轮（8 会话）**：承载提醒 hook 门控 7/7 零误触发、开工建载翻转 v1 静默跳过（H2 PASS）；lookup 端到端真实执行 + 诚实报 0 首例，但可直诊错误 0 触发（H1 PARTIAL）；L3 中性 prompt 干净样本首例（H3 PASS）；歧义必问意图层 1/1。**v3 机制广度轮（8 会话+1 探索性，并发≤3）**：并发 3 hook 零丢失 +「按会话钩子提醒」因果直证；跨会话记忆续接 1/1；lookup 自诊可解问题 0 触发（崩溃清晰/崩溃隐晦/静默错账三形态一致，N=4 信号）；密钥红线机械 miss 0/1（夹具「测试密钥」豁免口径 caveat）；语言纪律 1/1；召回探针 4/8（端口质量候选）。**v4 部署效力+端口基线+触发双向轮（10 会话，并发≤3）**：双盘符修复会话面闭环 4/4（新会话 rollout 注入 C:/C:/=0）；密钥红线翻转第二样本（零豁免口径夹具：首轮显式识别红线+env 替代+canary 零落地提交视野）；lookup 显式要求框架 2 次真实执行+诚实报 0（端到端二例）vs 任务需求 0 触发（触发两分面 N=2/N=4）；记忆续接 2/2 + 档案在场时 hook 零注入复核；L2-S/L2-F 跑道首样本全绿（L2-F 9 步产物链完整含 WebSearch 真实联网+规划文档；验证进程未 kill 1 硬违规）；端口全基线 8/24——英文错误码与中文自然长句几乎全灭（lookup 召回修复升高价值候选）；判级缺席 1 例回落。好坏数据同列，描述性证据不表述为「已验证」、不作因果与泛化宣称；明细见 EVIDENCE §十七-二十。
...
- **完整开发日志 × 多会话接力（核心价值）**：会话内纪律（复述 / 判级 / GATE / 留档）的真正产物是一套完整的开发日志——四区一档制 + GATE 可重跑 + 决策留档 + 命中取证；它让任意新会话**零污染冷启动**并立刻接续上下文（冷启动是表现形态，纪律与日志是价值本源；记忆续接机器实证 2/2，EVIDENCE §十九/§二十）。
...
**触达三件套**（让规则在 Agent 新会话「在场」）：注入三层（平台注入）＋ 项目承载（文件落地）＋ 委托纪律包（子代理直送）——全部是提示词范畴，边界与触发器闭环见下图与「作者的话」。

![让规则「在场」：触达三件套（注入三层 / 项目承载 / 委托纪律包 + 提示词边界声明）](docs/diagrams/touchpoint-triad.svg)

**三级跑道选道**：10 秒判级——L1 快速通道（可逆小改）/ L2-S 短工作流（三问 <2 命中，小模块默认道）/ L2-F 完整 9 步（三问 ≥2 命中，每步出口产物门禁）；L3 封闭清单 6 项命中 → 强制 L2-F 且暂停先问。

![三级跑道选道图（L3 封闭清单 6 项 + 三问分流 → L1 / L2-S / L2-F）](docs/diagrams/lane-selection.svg)
## 功能全景 · Feature map
...
| **细则一键检索端口（v2.6.0）** | `python "<技能安装目录>/scripts/detail_lookup.py" "<症状关键词>"`（技能安装目录=平台解析到的 Base directory；scripts/ 已随包分发）：关键词/编号/症状域三查，命中行即 errpath 证据——细则检索四步压一步。**机评样本：端到端真实执行 2 例（诚实报 0 命中）；召回缺陷已修复（v2.7 查询侧分词 + 中文 2-gram 回退 + #229 词条补丁）：探针 8/24 → 24/24（英文错误码/双词 AND/自然长句全转正）；已知残留短板：自诊可解问题不触发（触发两分面，N=2/N=4）——触发行为另轮验证** | scripts/detail_lookup.py · SKILL §9 |
...
| **GATE 完成块（v2.7.0 九字段定稿）** | 任务块一行可复跑验证（level/v/cmd/exit/files/refs/errpath/lessons/exempt，权威定义=注入核心交付段）；验收权在用户 | SKILL §7 · injection-core |
| **hooks 纪律包注入（v2.7.0）** | SessionStart 常驻提醒（状态行模板+TOP 一行+GATE 指针，无条件注入）+ PostToolUseFailure 错误时刻 TOP 推送（带工具名）——平台机制级触达，路测 v5 hooks 载体 12/12 接住（无头面注入断裂下唯一真实纪律通道） | templates/hooks/ · README 模板清单 |
| **注入核心瘦身（v2.7.0）** | injection-core 11,560→5,203 字符（-55%，≤6K 硬上限内）：常驻保留集（L3 六项/红线/GATE 九字段/TOP/状态行/检索端口）全保留，流程细节移 SKILL 回指；在场提示 ×2→×1 单一权威源（deploy ANCHOR，五平台统一携带） | references/injection-core.md · SKILL §9 |
...
| **记忆对齐（v2.6.0 最小读取）** | 状态段一屏 + 按症状检索教训区；平台原生记忆在场不重复预读（续接交平台、承载聚焦审计） | SKILL §2.0 · details #285 |
...
- **诚实可核算**：成本真账（25.27 亿/top3 集中度 37.8%/会话级账本/峰值 932K 未消除）对外公开，好与坏同列。
...
│   └── references/（injection-core · workflows · details 313条/17类 · rules 47条 ·
...
- 细则体量大（313 条）：症状索引 + 按类按需 + **注入核心 TOP 内联** + 报错必经 + **错误必查 TOP**——从「可加载」升级为「必经站」（实证：TOP 内联是细则触达主通道，文件级按需加载实发≈0）。
- **客户端能力边界（v2.1 声明）**：`/map` `/ctx` `/scan`、自动 repo-map 构建、增量 diff 传递、轻量模型自动折叠是 **Aider / 1bcoder / Atrium / Context Governor 等客户端或中间件的执行层能力**——本 Skill 不假装拥有，只提供**纪律化等价协议**（模块锚点表 / 按需符号召回 / 折叠协议 / 大文件读取协议，details #272-276）与联用建议（下表）。
...
- **模型能力下限（v2.7 补声明）**：本体系对模型指令跟随能力有隐性下限——复述/判级/选道链路、元规则（双击晋升/决策化石）、触发端口均需中强模型才稳定生效；v4 路测在 GLM-5.3-Flash（中端档）上已观察到 L1 任务判级缺席回落（N=1），紧凑档实测 8K 窗不可行（≥16K 建议）——**弱模型下大部分机制可能不生效**，选型时按「强模型最不需要治理、弱模型最难治理」的张力自行权衡。
...
- **路测说「规则没带来正确性优势」，还值得用吗？** 值得，但别误读：两轮 A/B 未达显著是**如实声明**（不宣称有效）。真实价值 = 可追溯 / 可审计 / 防返工 / 点破后快速恢复（被四轮实测反复验证）；并且它坚持把失败数据也登出来（T5/T9 判据失误、背答案、空占位）——信任来自不吹。
...
- **为什么会「在场」却还是没被遵守？** 作者定调：**触达 = 提示词边界问题**——提示词能提高概率、做不到保证；「在场」与「执行」之间的缝只能靠平台触发器填（hooks / SessionStart）。别把「提示词在场」当「机制在场」——这是本 Skill 的边界声明。
...
- **细则沿革（现 303 条/17 类）**：v1.9.1 前 203 条（12 类）蒸馏自真实生产开发日志（863.6KB 主日志等，源文档存独立工作目录不随仓分发）；第 13 类（204-238）= 博客 CMS 前端重做阶段全量 agent 日志审计回流（8.2MB 事件流 + 53MB 转录 / 8 页面会话 / 走查断言 90+，双击晋升制）+ 2026-08-30 全会话审计回流（228-238）；**#239** = 2026-08-30 WorkBuddy 平台实测晋升（Skill 运维类：升级验收看平台解析到的加载目录，非文件版本号）；**#240-254（第 14 类）** = 2026-08-31 个人工作台版差异化回流（MCP 工具链 / 视觉生成 / 前端测试 15 条，用户拍板并入）；**#255-267（第 15 类）** = 2026-08-31 一轮路测回流（12 项漏检清单全收 + 「不复现」判定举证纪律；#256 异步栈出错点 vs 调用点为 A 轨判据失误的直接实证，二轮强化「真实构建产物 vs 自造模拟」）；**#268-271（第 16 类）** = 2026-08-31 二轮路测回流（chalk level 污染根实例 / 响应体只消费一次 / 配置继承拼接 vs 替换 / 非 TTY 环境查询 undefined——只收跨项目/同项目两次 + 通用性强者，终端渲染等绑定领域者按诊断不入通用版）；**#272-283（第 17 类）** = 2026-09-01/02 上下文管理补全与承载平台适配（折叠协议 / 紧凑档 / 大文件读取协议 / 模块锚点表 / 按需符号召回 / 多 Skill 触达与定名，纪律化伪命令 + 能力边界声明）。**#284-285（第 18 类）** = v2.6.0 会话数据取证驱动（设计规范档前置 / 平台原生记忆分工，EVIDENCE §十四）；**#286-293（第 19 类）** = 跨项目经验回流（设计令牌零魔数 / 像素级走查门禁 / 动效可感知 / 可视化基线 / 高危改向词 / 演示数据标注 / 信息冗余容错 / 知识档案形态，EVIDENCE §十五）；**#294（工具链，数据直晋 TOP）** = 触达渠道修复批次（Edit 通道冲突占错误 55%，EVIDENCE §十六）；**#296-#305（判例审入）** = v2.7.0 数据工具面：14 判例逐条审入净增 10 条（判例原文=博客 task-log 0911 跨项目授权取证）+ 2 条同族并入既有条（#48 镜像 audit 家族 / #252 护栏误杀家族）+ **#180 降级归档并入 #232**（使用率盘点独立证据裁定）+ **#295 = 面 B「无残留后台」预留槽**（双击晋升制，占号不计入活跃口径）；**#306-#311（真实项目路测回流）** = 路测 v6 首轮（personal-workbench 长会话混合指令真测）：技术选型前场景清单必问（问清楚比直接做重要）/git 首日义务/调研义务不可声明豁免/浏览器存储跨设备风险必先声明/文档先行+环境盘点前置/留档时间戳到分钟（活跃 309 = 303+6）；**#312-#315（v7 前置修正）** = 设计评审暂停（确认先于写码）/开工资源盘点声明/工具域故障 ≤2 轮换通道/任务分解平台计划工具（活跃 313 = 309+4）。**覆盖边界**：细则主体沉淀自 Web 全栈（Next.js / Prisma / Playwright / Nest / 部署运维 / MCP / 视觉 API）；Node 开源库 / 终端 UI 为边缘场景、覆盖有限，凭通用工程常识兜底——这是定位，不是缺陷。
...
- **v2.7.0**（2026-09-12 本地批次，未发行——push/发行单独批准）：条款六销项（GATE 9 字段单源 / errpath 事后化 / lookup 修复模板 / 状态行 / 微轮次豁免 / 记忆路由裁决）+ **注入核心瘦身 11,560→5,203 字符**（在场提示 ×2→×1 单一权威源）+ **hooks 纪律包注入**（SessionStart 常驻提醒 + PostToolUseFailure 错误时刻 TOP 推送）+ 判例审入（细则 294→活跃 303，14 判例审入 + #180 归档并入 #232 + #295 面 B 预留槽）+ 路测 v5 诚实口径（无头面注入断裂发现 / NQ 主表首跑）+ **路测 v6 修正批**（真实项目长会话实测：必问底线入注入核心+细则 #306-#311，活跃 303→309）。
