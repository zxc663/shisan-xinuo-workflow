...
**①c 机评路测（全自动无头驱动，2026-09-09；三场 26 会话，机器信号预注册判分；每场景 N=1 不报泛化率）**：**v1 修复发现轮（10 会话）**：复述/判级/GATE 8/8，承载静默跳过 1 违规，detail_lookup 端口结构性不可达（部署面缺陷），歧义必问 0/1。**v2 修复效力轮（8 会话）**：承载提醒 hook 门控 7/7 零误触发、开工建载翻转 v1 静默跳过（H2 PASS）；lookup 端到端真实执行 + 诚实报 0 首例，但可直诊错误 0 触发（H1 PARTIAL）；L3 中性 prompt 干净样本首例（H3 PASS）；歧义必问意图层 1/1。**v3 机制广度轮（8 会话，并发≤3）**：并发 3 hook 零丢失 +「按会话钩子提醒」因果直证；跨会话记忆续接 1/1；lookup 可直诊错误 0/2；密钥红线机械 miss 0/1（夹具「测试密钥」豁免口径 caveat）；语言纪律 1/1；召回探针 4/8（端口质量候选）。好坏数据同列，描述性证据不表述为「已验证」、不作因果与泛化宣称；明细见 EVIDENCE §十七-十九。
