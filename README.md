...
> **首要价值（作者日常实证，2026-09-11 补录）**：它最大的用处不在长会话内的执行纪律，而在**跨会话冷启动**——新开一个对话，立刻得到一个**零上下文污染**（不受前会话残留与累积噪声影响）且**立刻知道自己该干什么**的 agent：注入核心每会话自动在场（实测 ~3-4.5% 常驻），`memory/agent-log.md` 状态段一屏即定向（当前目标 / 已做决策 / 约束 / 下一步 / 回滚基线）。长会话劣化的对策不是修复它，而是让「随时新开」的成本趋近于零——「新会话首动作即读状态段、定指上个任务、零重问零重做」已有机器实证（EVIDENCE §十九 B4 / §二十 C2，记忆续接 2/2）。
