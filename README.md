...
- **v2.5.0**（2026-09-08）：**留档一档制 + 设计前调研批次（本地批次，发行待批）**——①**留档一档制**（四 Skill 全修）：跨会话留档收敛为单文件 `memory/agent-log.md` 四区 + 唯一轮转档 `agent-log-archive-YYYY-MM.md`；state/experience/experience-mustread/preferences/task-log 五件套不再默认创建，旧项目「已就绪沿用不强制迁移」；injection-core/SKILL §2.0·§3·§10·§12/rules/workflows/bootstrap/memory-anchor/project-rules/模板全链同步（details 历史条由头部「留档落点映射表」覆盖）②**设计前先调研成熟设计**：设计类动作动手前先看 1-2 个成熟参照（官方示例/事实标准/同类成熟产出），并入设计铁律/复用铁律，不新增条款③**模板**：新建 templates/agent-log-template.md（四区骨架），workspace-memory-template.md 指针化④蒸馏版三份同批（主版/Single 1.2.0、Universal 1.1.0）。verify 6/6 PASS；本地 commit 不 push、发行面待批。
- **v2.4.0**（2026-09-08）：**三平台取证驱动修补 + 细则分层蒸馏批次（本地批次，发行待批）**——①**场景判定反转**（项目特征判据具体化=有 .git/多文件/既有 memory 或 docs 任一；判定不清→默认按持续先建承载兜底；单发判定须交付声明一行）②**细则三层结构**（T1 常驻=注入核心/T2 症状检索/T3 编号查询；details 头部层级导读 + 症状索引 19 域全标注）③**防棘轮**元条款（新增先 diff/重复合并/删减降级合法）④**消重**（SKILL §3 步6↔§2.0 步4 同源收敛）⑤**紧凑档窗口下限 ≥16K 转正**（8K 注入占比 75-112% 不可行）⑥版本 2.3.0→2.4.0。依据三平台取证（ZCode/Codex/WorkBuddy；注入前后判级 3.8%→23.5%、GATE 3.8%→14.7%、细则 0→17.6%；新发现 N1-N8 待审）。verify 6/6 PASS；本地 commit 不 push。
