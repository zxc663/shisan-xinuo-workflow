...
![version](https://img.shields.io/badge/version-2.6.0-blue) ![license](https://img.shields.io/badge/license-MIT-green) ![platforms](https://img.shields.io/badge/platforms-Codex%20%7C%20Claude%20Code%20%7C%20Cursor%20%7C%20Trae%20%7C%20Windsurf%20%7C%20WorkBuddy-orange)
...
> **首要价值（作者日常实证，2026-09-11 补录）**：它最大的用处不在长会话内的执行纪律，而在**跨会话冷启动**——新开一个对话，立刻得到一个**零上下文污染**（不受前会话残留与累积噪声影响）且**立刻知道自己该干什么**的 agent：注入核心每会话自动在场（实测 ~3-4.5% 常驻），`memory/agent-log.md` 状态段一屏即定向（当前目标 / 已做决策 / 约束 / 下一步 / 回滚基线）。长会话劣化的对策不是修复它，而是让「随时新开」的成本趋近于零——「新会话首动作即读状态段、定指上个任务、零重问零重做」已有机器实证（EVIDENCE §十九 B4 / §二十 C2，记忆续接 2/2）。**判分体系随之迁移（v2.7）：以「新会话初始化质量（NQ）」为第一指标**——首轮定向 / 记忆接续 / 零污染 / 定向成本 / 载荷正确性五个子指标全部机器可测，v4 数据回溯基线=首轮定向 8/8、续接 2/2、载荷 4/4；会话内纪律命中率降为 NQ 异常时的诊断面（定义见 docs/version-plan-v270.md「NQ」节）。

> **护城河（管道 × 水，2026-09-11 补录）**：冷启动协议是**管道**，`details.md` 的教训库才是**水**——新会话恢复出来的不只是「我在哪」，还有「我上次为什么在这、试过什么、什么行不通、下一步往哪走」。市面状态方案存的是**事实**（项目用 Prisma / 上次做到登录页），本库存的是**带事故现场、证据数字、根因与预防动作的失败记录**：#294 Edit/脚本双通道混用占全部工具错误 55%（186/337）、#233 命名直觉假绿附四类反例、#256 异步栈丢调用点出自一次真实 issue 误判——每条带技术栈标签、来源注记与命中计数，经路测跨池验证，按双击晋升 / 防棘轮双向流动。三要素乘积里，状态恢复与治理机制可被模仿，**「真实踩坑、如实记录、公开失败」的积累无法被模仿**（壁垒是时间差与意愿，不是秘密）。lookup 检索端口就是这只水龙头（v2.7 召回修复后 23/24）。覆盖边界如实声明：教训主体沉淀自 Web 全栈，Node 开源库 / 终端 UI 为边缘场景。
...
- **新会话冷启动（首要价值，作者日常实证）**：长会话劣化的终点不是修复而是新开——新对话天然零上下文污染，注入核心自动在场 + `agent-log` 状态段一屏定向，让新 agent「立刻知道自己要干什么」（记忆续接机器实证 2/2，EVIDENCE §十九/§二十）。
...
├── package.json（2.6.0）· docs/reference-sources.md · .github/workflows/（CI：verify-release）
...
- **细则沿革（现 294 条/17 类）**：v1.9.1 前 203 条（12 类）蒸馏自真实生产开发日志（863.6KB 主日志等，源文档存独立工作目录不随仓分发）；第 13 类（204-238）= 博客 CMS 前端重做阶段全量 agent 日志审计回流（8.2MB 事件流 + 53MB 转录 / 8 页面会话 / 走查断言 90+，双击晋升制）+ 2026-08-30 全会话审计回流（228-238）；**#239** = 2026-08-30 WorkBuddy 平台实测晋升（Skill 运维类：升级验收看平台解析到的加载目录，非文件版本号）；**#240-254（第 14 类）** = 2026-08-31 个人工作台版差异化回流（MCP 工具链 / 视觉生成 / 前端测试 15 条，用户拍板并入）；**#255-267（第 15 类）** = 2026-08-31 一轮路测回流（12 项漏检清单全收 + 「不复现」判定举证纪律；#256 异步栈出错点 vs 调用点为 A 轨判据失误的直接实证，二轮强化「真实构建产物 vs 自造模拟」）；**#268-271（第 16 类）** = 2026-08-31 二轮路测回流（chalk level 污染根实例 / 响应体只消费一次 / 配置继承拼接 vs 替换 / 非 TTY 环境查询 undefined——只收跨项目/同项目两次 + 通用性强者，终端渲染等绑定领域者按诊断不入通用版）；**#272-283（第 17 类）** = 2026-09-01/02 上下文管理补全与承载平台适配（折叠协议 / 紧凑档 / 大文件读取协议 / 模块锚点表 / 按需符号召回 / 多 Skill 触达与定名，纪律化伪命令 + 能力边界声明）。**#284-285（第 18 类）** = v2.6.0 会话数据取证驱动（设计规范档前置 / 平台原生记忆分工，EVIDENCE §十四）；**#286-293（第 19 类）** = 跨项目经验回流（设计令牌零魔数 / 像素级走查门禁 / 动效可感知 / 可视化基线 / 高危改向词 / 演示数据标注 / 信息冗余容错 / 知识档案形态，EVIDENCE §十五）；**#294（工具链，数据直晋 TOP）** = 触达渠道修复批次（Edit 通道冲突占错误 55%，EVIDENCE §十六）。**覆盖边界**：细则主体沉淀自 Web 全栈（Next.js / Prisma / Playwright / Nest / 部署运维 / MCP / 视觉 API）；Node 开源库 / 终端 UI 为边缘场景、覆盖有限，凭通用工程常识兜底——这是定位，不是缺陷。
