...
**① 工程消费与成本（平台聚合）**：细则层（现 309 条 / 17 类）**工程消费命中此前 = 0**（v1.13 全平台审计），v1.13-1.16 以「错误必经句 + 预读 TOP + 命中取证行」修复后转**有命中**（#228×3 / #229×2 / #233×1，sess_c0f4df2b 留痕）。成本实证（**2026-09-09 刷新**，2026-08-28→09-09 窗口）：ZCode 平台累计 **input tokens 2,526,616,450（25.27 亿）/ 9,737 次模型请求 / 108 会话**（output 737 万；与官方「应用用量」面板累计 25.3 亿**精确吻合**，本地与官方双口径交叉验证）——**好的**：108 会话**零 context_exceeded**；会话 input 集中度 top3 = 37.8%（对比 08-30 旧口径 81%——长会话病态集中已缓解，v2.x 上下文管理与留档机制启用同期）；**坏的**：单次请求峰值 input 932K、重试 356 次、单会话峰值 4.06 亿 input 仍未消除；工具调用 11,694 次 / 错误率 2.9%（**最高频坑 F8：Edit 通道状态冲突 186 次 = 全部错误 55%**——脚本批量与 Edit 工具混用所致，已立 #294 + TOP 内联；其余多为授权边界正常拦截）。**缓存语义（2026-09-09 二批补测）**：input 中 **98.2% 为 cache_read**（新鲜 input 仅 ~4,615 万 / 1.8%）——峰值 932K 实为 99.9% 缓存读，长会话重复上下文按缓存计价，成本叙事以「新鲜 token」为准（details 口径见 EVIDENCE §十四）。**注明**：25.27 亿是平台聚合，非 Skill 归因（归因明细见 EVIDENCE §九）；常驻开销实测 ~3-4.5%（~6-9K tok / 200K，2026-09-07 实测）；近 7 天（09-03→09-09）增量 input 13.18 亿 / 5,757 请求。
...
│   └── references/（injection-core · workflows · details 309条/17类 · rules 47条 ·
...
- 细则体量大（309 条）：症状索引 + 按类按需 + **注入核心 TOP 内联** + 报错必经 + **错误必查 TOP**——从「可加载」升级为「必经站」（实证：TOP 内联是细则触达主通道，文件级按需加载实发≈0）。
