...
> 更新：2026-09-15 ｜ 判级：L2-F v2.8.0 定调+审查修正+**全渠道发行**（独立审查 30 项 P0 当轮闭环/P1+P2 入 2.9；facts_sync 事实对账 verify 7/7；细则 330 条/17 类）｜ 签章：v2.7+ 重构施工计划已获用户全量批准+全部预授权（2026-09-12，批 0-5 分批自主推进）；**v2.8.0 已全渠道发行（2026-09-15，GitHub/Gitee/npm/ClawHub/About；回执见项目信息 §五）**。
...
- 当前：**v2.8.0 批次（2026-09-14 升板定调，2026-09-15 发行前准备完成，发行待批）**——2.7.2 修正批全部并入（每轮复述强制/回指加载/压缩接续 #326/系统级配置判级 #328/账目对账 #330/纯文档 commit #331/前置门 #332/post_tool_guard Bash 守卫/UserPromptSubmit 每轮再触达）+ 独立审查 30 项 P0 当轮闭环（P1×11+P2×16 入 2.9）+ facts_sync 事实对账（verify 7/7）；细则 330 条/17 类；五副本重部署 v2.8.0；RELEASE-CHECKLIST 重写 v2.8.0 待执行版。
