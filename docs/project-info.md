...
> 更新：2026-09-20（外部评审审计批 + 条款级落刀批 + **v3.2.0 发行批**） ｜ 判级：L2-F（判据版本化 j1.0→**j2.5** + 金样本回归 **23/23** + scorecard 指纹 + GATE 形态分型 + `scorecard_agg.py` + `JUDGELOG.md`；verify 8/8 含 H 项判据自测；细则 **373 条/30 类**〔编号至 #374〕）｜ 版本与部署状态：**v3.2.0 已全渠道发行**（源库 3.2.0 · 五平台注入 `--check` 5/5 v3.2.0/count=373 + `--check --hash` 5/5 `HASH-OK` · 六处技能副本 6/6 = 3.2.0；回执见 RELEASE-CHECKLIST L 节与 EVIDENCE §四十一） ｜ 沿革：v3.1.0 全渠道发行（2026-09-19）→ 2026-09-20 审计批 / 落刀批（#370-#374 + 四机检端口 + 判据 j2.5）/ 部署批 / v3.2.0 发行批 ｜ 签章：v2.7+ 重构施工计划已获用户全量批准（2026-09-12）；v3.0.0/v3.1.0/v3.2.0 已全渠道发行（回执见 RELEASE-CHECKLIST I/I-1/L 节与项目信息 §五）。
