...
## ② 目标规划（当前阶段）
- 已完成：v2.0.4 全渠道发行 → v2.0.5（路测回流+触达强化）→ v2.0.6（项目承载+授权边界）**均为本地批次，未推送未发行**。
- 待办：①发行面（dist/npm/GitHub Release/Gitee/ClawHub，另行批准）②下一轮路测（新会话以 v2.0.6 为基线：换项目池 + 转测触发率 + 项目承载验收点）③GitHub PAT 轮换（v2.0.3 起遗留最高优先）。
