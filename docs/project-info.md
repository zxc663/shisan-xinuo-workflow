...
- 待办：①GitHub PAT 轮换（v2.0.3 起遗留最高优先）②ClawHub 1.0.10/1.0.11 审核复查 + skills.sh 遥测收录核查 ③下一轮路测（基线 v2.3.0，验收点含场景判定/errpath 生效/索引表 1-2 步定位）④注入副本重部署 v2.3.0（待批）——v2.3.0 发行面已于 2026-09-03 完成（dist/npm/Release/ClawHub/About）。
...
| dist/ | 已实现（gitignore） | 发行 zip（最新 v2.3.0；v2.4.0 待发行时打包） | 发行 zip、发布包、dist |
