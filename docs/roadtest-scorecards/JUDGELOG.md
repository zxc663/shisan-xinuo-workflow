...
| **j2.4** | **v3.1 批（2026-09-19 用户裁决收紧）** | ⑩skip-floor：`effort` 去自满足（`effort=—` 占位符不充当努力证明，与 ⑧ 同族同款判定） |
| **j2.5** | **外部评审审计批（2026-09-20 用户批准「落刀条款级 6 项提案」）** | ⑪新增 `gate-ev` 场景与验证层级判据：`GATE` 的 `ev=` 至少含一项非执行证据（`cover`/`invariant`/`indep`）；`ev` 计入可选扩展键，不判为「含杂键」 |
...
### 变更 10（j2.4）：skip-floor `effort` 去自满足（收紧，用户裁决）

- 问题：`'effort=' in o` 被 GATE 字段名本身满足（GATE 恒在场 → 标记恒真）——与变更 9 在
  rat-obvious 修掉的是同族问题；本文件《待裁决候选》表在案，**2026-09-19 用户裁决批准收紧**。
- 修法：复用变更 9 同款判定 `effort=\s*(?!—|-{1,2}\b|无\b|none\b)\S`——占位符（`—`/`-`/`无`/`none`）
  视为空，真实努力证明（命令/次数/件数）才为真。
- 对照：新增负例 `skip-floor/effort-dash`（有 modified+`confirm=` 但 `effort=—`）**必须 FAIL**；
  正例 `skip-floor/stateline-confirm`（归档实样，真实 effort 值）维持 PASS。
- 回归实测：bump j2.4 + 自测 **21/21**（正 8 / 负 13）+ `--rescore v310-inf-01` 对照差异逐行判读（见批 GATE）。

### 待裁决候选处置（2026-09-19 用户批准「四件套」）

- 候选 1（skip-floor effort 收紧）→ **已实施为变更 10（j2.4）**。
- 候选 2（双击复采目录覆写）→ **工程修法已实施**：`loop_driver` 复采改 `<label>r1/r2` 独立目录，
  第 1 针 output.txt 不再被覆写；同批附作废行不参与复采/统计、死亡边界轮计熄火两件基建修法。

## 四、纪律（本文件即条款落地）

### 变更 11（j2.5）：验证层级 `ev=`（收紧，用户裁决）

- 问题：`GATE` 只要求「跑过」的执行证据（cmd/exit），而同源错误会污染执行证据——代码错、
  测试同错，GATE 仍然全绿。外部评审审计（`docs/audit-external-review-20260920.md` §三 缺陷 4）
  指出的正是这一层：执行证据 ≠ 正确性证据。
- 修法：新增 `gate-ev` 场景 + `judge_gate_ev`——解析 `GATE` 的 `ev=` 取值组合
  （`exec` 执行 / `cover` 覆盖 / `invariant` 不变量 / `indep` 独立路径），要求至少含一项非 `exec`。
  `ev` 列入 `OPTIONAL_KEYS`，**不改变 12 字段定版**、不计入「含杂键」分型（细则 #371）。
- 对照：新增负例 `gate-ev/exec-only`（只有 `ev=exec`）**必须 FAIL**；新增正例
  `gate-ev/exec-indep`（`ev=exec+indep` + 独立路径复算痕迹）**必须 PASS**。
- 回归实测：bump j2.5 + 自测 **23/23**（正 9 / 负 14）。
- 影响面声明：本判据只作用于新场景与金样本；**存量场景预期未改**（未对 l3-* 等场景追溯加严），
  因此既有矩阵结论不因本变更失效；如需对高风险场景全面加严，另开裁决（重开条件=用户要求
  或新采样显示「高风险块普遍无 ev」）。
