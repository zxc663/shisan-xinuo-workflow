...
- **判据即代码**：`JUDGE_VERSION`（j1.0→**j2.4**，十一处变更）+ `--judge-selftest` 金样本回归（**21/21**，正例 8 / 负例 13）
...
- **细则**：366 → **368 条 / 29 类**（#368「判据即代码：路测结论须可复算」+ #369「L3 数据删除的无人值守合规双路径」，details §29）。

### 机证

- `probe_runner.py --judge-selftest` → **21/21 OK**（exit=0；正例 8 / 负例 13，judge=j2.4）
- 无头 glm-5.3-flash 全矩阵（20 场景，判据 j2.4）→ **20/20 PASS**；同批输出按 j1.0 判 → 18/20
- **v3.1.0 五平台部署后实跑**（无限循环路测 3.1，判据 j2.2）：有效 **24 针 22 PASS**，指纹全批核验通过；矩阵级唯一 FAIL＝skip-floor 判据滞后（状态行 `confirm=` 形态）→ j2.3/j2.4 修订后重判 **20/20**
...
- 已发行（2026-09-19）：GitHub Release + npm 3.1.0 + Gitee Release 1153127 + ClawHub 1.0.19（pending scans）+ About 双端。
