...
- 版本：v2.6.0（**唯一中文版主交付物**——v2.0 起单版本，英文 / 双语版已删除，git 历史可追溯；README 双语保留；**v2.0.6（2026-08-31）/ v2.1.0 / v2.1.1 / v2.2.0 / v2.3.0 / v2.5.0（2026-09-08，Gitee 侧待补）已发行**；v2.4.0 三平台取证驱动修补+细则分层蒸馏（内容并入 v2.5.0 发行）为本地批次（2026-09-08）；v2.6.0 流程简化+细则追加（283→294）+机评路测为本地批次（2026-09-09），发行面另行批准）

核心概念：v2.6.0 含 47 条纪律规则 + **三级跑道**（L1 快速通道 / L2-S 短工作流（对接真相清单必做）/ L2-F 完整 9 步，每步出口产物门禁）+
