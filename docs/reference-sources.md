...
> 目的25eepSeek、豆包、Kimi 等网页 AI 不会"记住"这个 2026 年新建的仓库，其题目内置知识有截止日期、默认联网搜索很少抓 GitHub。**正确用法**：把下面的权威直链连同定位文本一起粘贴给网页 AI，它会对这些 URL 发起联网读取，得到真实、当前的仓库信息。
...
- 版本：v2.9.0（**唯一中文版主交付物**升板，修正批：独立审查 P1×11+P2×16 机制级 23 项+v11 新增四项裁决（#312 增量豁免/#326 适用性/#327 裁决定版/注入快照探针）+G 清单四点调研立条（#333 文档分型/#334 依赖锁文件/#335 性能预算+security 应用安全基线节），活跃细则 367（修正批 #333-#335+G 直写批 #336-#344+#295 双击转正+v3.0 批① 立条 #346-356〔未发行〕）；上一版 v2.8.0（2026-09-15 全渠道发行）：每轮复述强制+压缩接续义务+GATE refs 实测+系统级判级+增量解释+回指加载，330 条；上一版 v2.7.1 补丁：全渠道介绍文案同步 v2.7.0 完整口径；历史主版本**v2.7.0**：**唯一中文版主交付物**——v2.0 起单版本，英文 / 双语版已删除，git 历史可追溯；README 双语保留；**v2.0.6（2026-08-31）/ v2.1.0 / v2.1.1 / v2.2.0 / v2.3.0 / v2.5.0（2026-09-08，Gitee 侧待补）/ v2.6.0（2026-09-09，Gitee 侧待补）已发行**；v2.4.0 三平台取证驱动修补+细则分层蒸馏（内容并入 v2.5.0 发行）为本地批次（2026-09-08）；v2.7.0 全程批次（条款六销项+注入核心瘦身 5,991+hooks 纪律包+判例审入+必问十维扩充+评审批 #312-316+减法批+口径净化批，活跃细则定稿 314）已发行（2026-09-12，GitHub 侧；Gitee 已补发））

核心概念：v2.9.0 含 47 条纪律规则 + **三级跑道**（L1 快速通道 / L2-S 短工作流（对接真相清单必做）/ L2-F 完整 9 步，每步出口产物门禁）+
367 条落地细则（29 类）+ 渐进式披露 + 会话钩子示例 + 一键安装脚本（agent- 前缀自适配）+ 硬注入三层承载（记忆层在场提示 + 规则层 + 配置层）+ 发布一致性校验脚本（内容锚点门禁 + 正文净化检查）+ 上下文主动管理（折叠协议 / 保留清单 / 紧凑档 / 模块锚点表 / 按需符号召回 / 大文件读取协议 / 决策时效）。
