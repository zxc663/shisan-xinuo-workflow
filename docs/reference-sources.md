...
- 版本：v2.7.0（**唯一中文版主交付物**——v2.0 起单版本，英文 / 双语版已删除，git 历史可追溯；README 双语保留；**v2.0.6（2026-08-31）/ v2.1.0 / v2.1.1 / v2.2.0 / v2.3.0 / v2.5.0（2026-09-08，Gitee 侧待补）/ v2.6.0（2026-09-09，Gitee 侧待补）已发行**；v2.4.0 三平台取证驱动修补+细则分层蒸馏（内容并入 v2.5.0 发行）为本地批次（2026-09-08）；v2.7+ 重构六批次（条款六销项+注入核心瘦身 5,203+hooks 纪律包+判例审入活跃 303+收口定稿+路测 v6 修正批+v7 前置修正+接手全景 #316 活跃 314）为本地批次（2026-09-12），发行面另行批准）

核心概念：v2.7.0 含 47 条纪律规则 + **三级跑道**（L1 快速通道 / L2-S 短工作流（对接真相清单必做）/ L2-F 完整 9 步，每步出口产物门禁）+
303 条落地细则（17 类）+ 渐进式披露 + 会话钩子示例 + 一键安装脚本（agent- 前缀自适配）+ 硬注入三层承载（记忆层在场提示 + 规则层 + 配置层）+ 发布一致性校验脚本（内容锚点门禁 + 正文净化检查）+ 上下文主动管理（折叠协议 / 保留清单 / 紧凑档 / 模块锚点表 / 按需符号召回 / 大文件读取协议 / 决策时效）。
