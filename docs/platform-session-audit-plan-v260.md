...
## 2. 逐会话判分口径（v2.7 起：NQ 第一指标打头，纪律命中率降为第二层）

每会话一张 scorecard，**主区=新会话初始化质量（NQ，定义见 docs/version-plan-v270.md「NQ」节）**：

0. **NQ 主区**：①首轮定向（复述/理解陈述在场）②记忆接续（有档续接零重做 / 无档建载不缺位）③零污染（注入占比+零串写）④定向成本（首轮即定向的请求数）⑤载荷正确性（锚点/版本/无病灶）
   ——纪律命中率作为 NQ 异常时的诊断面放次区：
