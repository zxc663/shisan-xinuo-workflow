...
## 0. 任务主流程——调研驱动（任何任务类型前先执行）

- [ ] 1. 接收指令；第一性原理理解（本质 / 必要 / 惯性）
- [ ] 2. 按症状 / 关键词检索经验库；命中 → 按「解决 / 预防」执行
- [ ] 3. 调研实际资源：真实代码（现状证据：消费方 / 常量 / 开关 + 文件行号结论）、运行时环境、工作区、本次会话可用 Skill 与 MCP
- [ ] 4. 降级：环境 / 能力 / 工具 / Skill / MCP 不可用 → 联网调研（文档 / 库 / 等价物）、加载所需 Skill、记录降级与原因
- [ ] 5. 复用调研：本地项目资源 → 同类 / 已知项目资源（复用五问）；自研时记录结论
- [ ] 6. 向用户复述理解（目标 / 边界 / 验收）；确认对齐
- [ ] 7. 疑问或方向偏移（范围 / 冲突 / 歧义 / 风险）→ 提问并结束回合等待
- [ ] 8. 产品视角审查 + 显式化约束 / 假设 + 定级 L1/L2/L3 + 准备回滚点（规则 43）
- [ ] 9. 规划与验收文档：3-5 条可验证标准；目标模式另加时间 / 轮次 / 预算上限 + 文件边界拆分
- [ ] 10. 按分级执行（目标模式：按计划自主、节点记录、超预算自动停）
- [ ] 11. 自查与归档：最小验证 → 自查 → 文档与代码同批 → 双写知识 → 提交推送附说明
