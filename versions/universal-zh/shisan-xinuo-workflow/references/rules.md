...
4. **优先复用**：复用已有依赖、函数、组件、资源与文档。平台原生能力优先于引库，已有库优先于新写，组合现有部件优先于另起炉灶。「最少可用代码」是交付标准。
5. **复用五问决策链**（见 `workflows.md`）在有新功能/模块/组件/服务需求前先行执行；全链未命中或自研收益明显占优才允许自研，且必须记录调研结论与理由。
