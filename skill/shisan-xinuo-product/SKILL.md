...
description: "产品工程包（联通层/结构层核心）：把「功能需求→交互逻辑→实现」钉成可判定契约——功能天生六问翻译表、态+转换 statechart、双底双顶（不难用/不难看/无障碍底线，更好用/更好看上限）、spec-trace 双向追溯、裁决账本写回、registry 组件归因门禁。本包=交互工程（Product Contracts 规范层的第一垂直域）；母架构其余域（Intent/Structure/Runtime/Implementation/Verification）**未实现**。管「这个功能应怎么来」；手法（怎么做才好看）归各设计 Skill。Use when：前端/功能设计任务，或用户说「难用/丑/反直觉/不好用」「交互逻辑怎么来的」「功能怎么变界面」「为什么这样交互」「假功能/点了没反应/写了没接」「把 JSON 甩用户脸上」「重复造轮子/不用现成组件/另造一页」「设计稿和实现不一致」「空态没有引导/错误没有出路」「高密度界面乱/层级乱」，或审设计稿、验收交互、设计走查。Not for：纯视觉调色、纯后端、与用户交互无关的任务。"
license: MIT
