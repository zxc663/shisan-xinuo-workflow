...
description: "产品工程包（联通层）：把「功能需求→交互逻辑→实现」钉成可判定契约的结构层核心——功能天生六问翻译表、态+转换 statechart、双底双顶（不难用/不难看/无障碍底线+更好用/更好看上限）、spec-trace 双向追溯门禁、裁决账本写回。管「这个功能应怎么来」；手法（怎么做才好看）归各设计 Skill。适用：前端/功能设计任务、「难用/丑/交互逻辑怎么来的/功能怎么变界面/为什么这样交互/假功能/点了没反应」；不适用：纯视觉皮肤、组件库选型本身、与产品交互无关的任务。"
license: MIT
