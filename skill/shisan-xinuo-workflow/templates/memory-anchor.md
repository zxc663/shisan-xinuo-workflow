...
- 项目承载（自动建，不等许可）：memory/ 规范五件套（state.md … task-log/，禁空占位）+ 项目级规则文件（按 platform-adaptation.md「项目级注入点表」定名，templates/project-rules.md，先查既有→合并不覆盖）+ docs/project-info.md。
- 委托子代理：必须内联纪律包（子代理不继承注入副本、不保证自加载 Skill——实测实证；独立工作区另建规范承载）。
- 完整规则：规则层文件（AGENTS.md / user_rules / CLAUDE.md）+ 技能 references/（rules.md 47 条 / 283 细则）。
