...
- 完整规则：规则层文件按平台各异——AGENTS.md（Codex/ZCode/WorkBuddy）/ CLAUDE.md（Claude Code）/ .trae/rules/project_rules.md（Trae）/ .cursor/rules/*.mdc（Cursor）/ .windsurferules（Windsurf）/ user_rules；细则 references/（rules.md 47 条 / 365 细则），全表=platform-adaptation.md §2 注入点表。
