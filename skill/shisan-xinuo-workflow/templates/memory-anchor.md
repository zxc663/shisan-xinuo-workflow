...
- 完整规则：规则层文件按平台各异——AGENTS.md（Codex/ZCode/WorkBuddy）/ CLAUDE.md（Claude Code）/ .trae/rules/project_rules.md（Trae）/ .cursor/rules/*.mdc（Cursor）/ .windsurferules（Windsurf）/ user_rules；细则 references/（rules.md 47 条 / 330 细则），全表=platform-adaptation.md §2 注入点表。
- 更新协议：`python scripts/syncer.py`（记忆/规则/配置三层随版本同步）；验收以平台解析到的 Base directory 为准。
- 注入版本：vX.Y.Z ｜ 授权：本锚点由用户授权后注入，未获授权不写。
```

（替换 `vX.Y.Z` 为当前版本号；正式写入后本模板保持 `vX.Y.Z` 占位，便于反复复制。）