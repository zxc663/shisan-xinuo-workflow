...
description: "一句话定位：把任何工程任务强制按「三级跑道（L1 快速通道 / L2-S 短工作流 / L2-F 完整 11 步）+ L1/L2/L3 封闭清单速判 + 三模式」推进的可审计 Agent 工程纪律工作流，核心纪律可平台无关硬注入。任何动手任务开工必用。"
license: MIT
...
6. **有效性诚实声明**：本工作流「规则改变行为」的有效性证据 = 两轮 A/B，均**未达显著**——2026-08-30 一轮 4 任务迷你实验（四项指标无差异）；2026-08-31 二轮路测（6 开源项目真实 issue，A 轨 n=14 / B 轨 n=3，跨任务异构难配对，**不出 p 值**）：仍**未观察到「带规则」带来正确性优势**，且 A 轨出现一次关键判据失误（issue 复现判据用错对象）被无规则轨独立纠正——机制按既定拍板保留，但不得宣称已证明有效；**已知触达局限（比命中率更值得关注）**：规则在场 ≠ 规则被遵守，高密度任务流下流程性纪律（块尾记录 / GATE / 五问）会被自发跳过（二轮路测必产物 0/3 实证），下一轮验证须显式要求逐块产出并选 L2 级任务。
...
- **判「不复现」需要更强举证**（2026-08-31 路测 T5 反例）：逐字核对 issue 标题所指对象（「uses」指调用点不是出错点）+ 对照实验（直接调用 vs 经库调用各跑一遍）；自写模拟缺真实结构 → 结论降「未定论」，不判「不复现」（details #255 / #256）。
...
| `references/details.md` | 落地细则（工程规范 / 具体做法）——15 类，症状索引 | 症状关键词命中踩坑类别（构建工具链 / 框架版本 / 无障碍 / API 形态 / 部署 …）——**在不熟悉领域即兴发挥前，先查这里** |
...
**错误时强制入口**：遇到错误 / API 意外形态 / 未知字段或端点 / 新依赖不生效 → **先按症状类搜 `details.md`**（如 build-tool、[Contract]、[Ops]），再改代码；命中 → 任务记录留一行引用——**统一写完整前缀形态 `details #N 命中：…` / `细则 #N`，禁裸 `#N`**（与 GitHub issue 编号无法区分，2026-08-31 路测实证假阳性 9/10）。
