...
description: "25话定位：把任何工程任务强制按「三级跑道（L1 快速通道 / L2-S 短工作流 / L2-F 完整 9 步）+ L1/L2/L3 封闭清单速判 + 三模式」推进的可审计 Agent 工程纪律工作流，核心纪律可平台无关硬注入。适用：任何动手工程任务——编码/多文件/跨模块/修 bug/建组件/审查/用户点名按流程；不适用：纯聊天问答、单文件只读查询、与工程无关的对话。"
license: MIT
...
- **口径关系**：注入核心=常驻最小集，本文件=全文展开；二者必须同义，冲突以核心为准并立即修本文件（细则 #325 同源）。
- **复述前置（首产物状态行 + 阶段边界复述）**：开工先严谨复述目标/边界+规则要点再动手——不自以为是「理解了就理解了」；并输出一行可 grep 状态行 `Context: state=<读档/新建/单发> L=<L1/L2-S/L2-F> confirm=<无需/已问/豁免:理由>`（判级并入 L= 字段）。理解缺口 → **先补依据**（澄清提问或现场查证）再继续。**每一用户对话轮的首产物=复述**：新任务全量三行；追加/继续/追问须一行复述（收到 X｜理解为 Y｜边界 Z）——用户须随时校验理解，无例外；阶段边界（开工 / 收尾 / L3 决策点 / 方向变化）加全量复述；**L3 确认不豁免**；摘要接续须一行接续声明（跳过+声明合法）；L1/安静模式复述并入状态行，不另展开（防 token 浪费）。
- **文档写作分层（通用纪律，详见 §10 总纲）**：写任何文档产物时，正文只写结论/规则 + ≤1 句「为什么」，史料（出处/拍板人/日期/版本/轮次）落决策史层——依据落盘先对号入座（details #278）。
- **防棘轮（本体维护原则）**：本 Skill 正文与细则以「语义不重复、检索面收敛」为维护目标，不以条数增长为成果——新增条款先 diff 比对既有、重复当场合并，删减与降级合法且鼓励（细则层维护规则见 details.md 头部）。
...
**使用**：任何工程任务（只要动手干活，就按第 2 节跑道推进）；用户要求纪律化执行、工作流治理、工作流规则，或希望 Agent 在跨项目、跨平台时行为一致。本 Skill 每次加载时也会执行第 3 节第 0 步（平台适配）。**加载即场景判定 + 承载检查**：本 Skill 被加载 / 被用户显式调取时——**无论注入核心是否在场**（多 Skill 共存平台，在场的注入核心可能是其他 Skill 的，本 Skill 条款不可依赖它触达）——立即按 §2.0 场景判定执行（单发/持续分支、判定不清默认按持续，**以 §2.0 为权威，此处不重复**）；不等用户要求。
...
> **这是本 Skill 的核心，不是参考项。** 每个任务必须按此流程推进。**10 秒选道**：先按 §5.2 判级速查的分流行回答三问 → 命中 L3 任意项或三问 ≥2 命中 → **L2-F 完整 9 步**（§2.4）；其余、且满足触发条件 → **L2-S 短工作流**（§2.3）；L1 → 快速通道（§2.2）。每次选道在任务记录里留一行。

### 2.0 开工序列四步（每次任务开工统一入口；每步有出口产物，无产物不进下一步）

**场景判定（先定单发还是持续项目，再走四步）**：**项目特征判据**——工作区有 .git/仓库、多文件、既有 `memory/` 或 `docs/` 任一即算项目特征。**单发使用**——工作区无项目特征**且**任务非工程性质（问答/分析/检索/单点修改）→ **纪律全走、文档豁免**：复述/严谨分析/判级/红线照常执行，但第 2 步承载检查跳过——不建 memory 一档/规则文件/docs，**交付回复声明一行「单发模式：未建承载」**（供用户纠偏）。**持续项目**——工作区有项目特征任一，或任务工程性质（多文件/跨模块/长期迭代），或用户点名项目 → 四步全套+承载创建强制（含回指）。**判定不清 → 默认按「持续」处理**（先建承载兜底，宁多留档不少留；确认单发后删除承载文件即可，可逆——误判持续的代价是删几个文件，误判单发的代价是任务中途扩展后丢初始上下文，后者更贵）。

| 步 | 动作 | 出口产物 |
|---|---|---|
| 1 | **复述理解+状态行（无条件第一步，含 L1 与单发）**：严谨复述指令目标/边界+规则要点——不自以为是「理解了就理解了」；并输出一行可 grep 状态行 `Context: state=<读档/新建/单发> L=<L1/L2-S/L2-F> confirm=<无需/已问/豁免:理由>`。理解缺口 → **先补依据**（澄清提问或现场查证）再继续。现状/目标模糊 → §2.1 状态澄清；**摘要接续的会话走 §0 接续豁免（跳过+一行声明合法，L3 确认不豁免）** | 状态行 + 复述（用户可纠偏） |
| 2 | **承载检查（扫描 → 定根 → 建补，一气呵成；持续项目强制 / 单发豁免）**：列根目录一层（>20 项或超大仓库 → 只列目录名计数+按任务关键词探一层）→ **形态判定**；多目录混杂 → 按任务焦点定承载根（留一行「承载根=X，依据 Y」，真歧义才问；可疑 → 停下确认不静默落盘）；无 `memory/agent-log.md` → 自动建一档制单文件（四区骨架从 `templates/agent-log-template.md` 复制，**禁空文件占位**；旧五件套项目已就绪沿用——**收敛口径：agent-log 作入口档，状态段/教训区以指针回指权威 memory 体系，仅流水区增量，勿另起平行体系**）；无 docs/project-info.md 且多文件 → 建索引式六节（§2.5）；无项目级规则文件 → 按 `platform-adaptation.md` **项目级注入点表**定名自动建（先查既有：合并不覆盖+先备份；**回指本 Skill = 强制字段，缺失 = 不合规**——同空文件占位）——**不等许可**；已就绪 → 增量补缺。**版本控制开局（按场景）**：空目录新项目=`git init`+初始 commit；接手他人仓库=先 `git log`/`status` 摸底+把现状 commit 为基线（干净工作树）再动手，改动前留回滚点（细则 #307） | 形态判定 + 承载就绪（创建清单或「已就绪」或「单发模式：未建承载」） |
| 3 | **记忆对齐（最小读取）**：读 `memory/agent-log.md` 状态段一屏；教训区 T1/T2、偏好段、流水区**按当前症状精确检索命中条目，不整篇载入**——平台有原生记忆机制（auto-memory / 记忆文件）时不重复预读：跨会话偏好与事实续接交给平台记忆，本档聚焦项目审计 | 读取记录（或「平台记忆在场，按症状检索」一行） |
| 4 | **开工前置门+判级选道**：主流程启动前过前置门四项（调研基准/主动加载 Skill/MCP 盘点/回滚点——细则 #332，L1 豁免须留痕）→ §5.2 判级速查 + 三问分流 → 三级跑道 | 前置门留痕 + 选道一行 |

**L1 豁免**：整体标注「L1 精简」→ 承载检查豁免、承载增量补缺不豁免（已就绪零动作）；**状态行+复述不可豁免**（§0 复述前置；L1 复述可并入状态行一行）。
...
**跳步豁免**：L1 快速通道跳过的中间步（第 2-8 步）**豁免**「无法产出的步须逐一写明理由」的门禁——整体标注「L1 快速通道」一次即视为已登记理由，不为每步单列；开工序列四步同步豁免（§2.0 L1 豁免——承载检查免，承载增量补缺与复述不免）。
...
|---|---|---|
| 1 | **接收指令**：第一性原理理解——本质 / 必要 / 惯性 | 任务本质一句话（本质/必要/惯性三拆＋系统约束一句——出口产物即思考链自检位，缺三拆或约束=步 1 未过） |
| 2 | **调研实际资源**：真实代码（现状证据）+ 结构 / 契约 + **对接真相表（§2.3）** + 模块清单（**真实状态**：已实现 / 规划中 / 未实现）+ 环境 + 可用 Skill / MCP；**教训区按症状检索**（命中记一行，0 命中 → 记「无既往经验」——不整篇预读） | 现状事实清单（含文件 / 行号 / 结论 + 教训检索行） |
| 3 | **双调研一次完成**：①联网调研——开源成熟项目 / 库 / 方案 + **可验证的可信信号**（stars / 下载量 / 维护活跃度 / 被采用证据，不以「网上都说火」为依据；细节见 `references/workflows.md` §0.2；**离线 / 无网络是合法降级**：跳过远程调研，产物标注 `degraded-offline`）②复用调研——本地项目 → 市面开源成熟项目，能复用就复用（复用五问），**绝不自己自研组件**。**设计类动作前置（前端尤甚）**：逐组件调研成熟规范（官方文档 / 事实标准 / 机读设计系统：组件 API / tokens / 交互态 / 可访问性基线），产出**强制留档** `docs/design-specs/<主题>.md`（或既有设计系统档追加，禁只留在对话里），实现**按档设计并回指**（details #284） | 市面方案调研记录（候选 + 可信信号 + 降级情况）+ 复用结论（或设计规范档 + 回指行） |
| 4 | **复述理解**：目标 / 边界 / 验收口径 | 用户确认（对齐后继续） |
| 5 | **疑问必问**：执行不理解或方向偏移 → 提问并结束回合等待（§4） | 提问 / 确认记录 |
| 6 | **产品视角审查 + 约束假设 + L1/L2/L3 分级 + 回滚点**：以产品角度审视（体验与表现优先）；**产品视角五问（L2/L3 计划默认强制）**——诉求一句话拆解 / 被否候选 ≥1 / 返工成本 / 边界与不做的清单 / 3-5 条可验证验收；触发「反复要求审查 / 存量项目反复不足 / 知道不够好但说不清」时，**先做产品完善度诊断**（五问定位缺陷落在：功能逻辑 / 代码耦合 / 界面 UI / 人性化互动流程 / 其他，见 workflows.md §0.3）再动手 | 五问记录 + 风险分级 + 回滚点记录 |
| 7 | **规划与验收文档（双调研后）**：先完成**双视角调研**——①工程师视角（代码实况 / 技术可行性 / 复用，见第 2、3 步）②产品经理视角（当前设计规划的合理性：本质需求 / 设计是否完备 / 体验·UI·交互是否符合产品定位，见 workflows.md §0.4）——再产出**详细规划文档**（双调研结论 + 功能清单优先级 + 3-5 条可验证验收标准；目标模式加预算与文件边界） | 详细规划文档（含双调研结论） |
| 8 | **执行**：按分级执行；目标模式按计划自主、节点记录、超预算自动停 | 执行记录 / 变更 |
| 9 | **自查与归档**：最小验证 → 真实用户走查一遍（不可用改到可用，允许多轮）→ 交付五查 → GATE 块 → 留档（§10）→ 提交附说明 | GATE + 验证结果 + 归档 |

**门禁**：进入下一步前，上一步的出口产物必须存在且已记录；无法产出的步（如明确无需调研）也要在任务记录里写明理由，不得静默跳过。

### 2.5 新项目 / 首次加载——项目信息文档

**触发**：工作区根**无 `memory/` 骨架** 或 无 `docs/` 指导（或碎片化）——**触发是存在性判定，不取决于「算不算新项目」**（实测：大型仓库仍会因判定犹豫而漏建承载——存在性判定防犹豫）。
...
0. **安装名前缀自检（每会话触发）**：每个会话开工（识别到在场提示 / 硬注入核心 / 本 Skill 加载时）执行一次——**能确定本 Skill 安装目录名（Base directory，来自平台系统提示、技能列表路径、状态面或用户告知）**时：无 `agent-` 前缀 → 一行提示「可运行 `scripts/install-skill.ps1` 一键带前缀重装（自适配到目标平台技能目录）；不愿改名则靠 description 触发，按需注入平台不强制」；带 `agent-` 前缀 → 静默通过。**两个合规出口**：①无法确定目录名 → 不猜测、不阻塞任务，状态面记一行「前缀自检：目录名未知，跳过」；②用户已明确选择不改名 → 在 agent-log 状态段记一行「保持无前缀」，此后本自检静默。前缀意义：技能列表按字母序置顶，便于按需注入用户发现（README FAQ）。
   > 触达承载面（实证：自检只写在 §3 时长期 0 触发——仅靠按需加载无法保证每会话触达）：本条权威源在 SKILL §3，**执行依赖每会话在场的两层承载**——injection-core.md 开工序一行（硬注入规则层）+ `templates/memory-anchor.md` 锚点行（记忆层）。
...
3. **询问注入模式**（写规则文件前，用 §4 降级链提问，让用户选择）：
   ...
   - **强制注入（硬加载）**：**三层同时写**——记忆层锚点块（`templates/memory-anchor.md`，首行为「在场提示 · 工作流 Skill 现已在场」——新会话读到即识别本 Skill 在场）＋ 规则层（把 `references/injection-core.md` 的核心全文写入规则文件）＋ 配置文件层（hooks/全局设置，平台支持才写，§2.1）——每步**先备份既有文件、合并不覆盖**。写前必须**提醒用户授权**（平台 / 三层目标 / 内容长度 / 每会话成本 / 影响范围）。工作流每会话无条件在场，不依赖模型自觉加载本 Skill（常驻核心有字符预算硬上限，瘦身后 ≤6K 字符）。**注意：不要用「每个会话开工前必须完整读取本 Skill 的 SKILL.md」这类弱指令实现强制注入——模型不会可靠执行，必须直接写入核心全文与在场锚点。**
...
6. **项目承载检查（部署第五层）**：与 §2.0 开工四步第 2 步（承载检查）**同源同规则（彼处为权威，此处不重复清单）**——部署时点校验同一承载三件：memory 一档（`memory/agent-log.md`）/ docs 六节（多文件时）/ 项目规则文件按 `platform-adaptation.md`「项目级注入点表」定名 + 回指强制（合并不覆盖+先备份）；创建自检四项（details #282：定名按表 / 回指在场 / 承载根非临时未命名 / 汇报路径——实测：承载清单不列项时模型只建 memory/ 不建规则文件，文件名必须查表定）。均自动动作、无需授权；授权边界见 §4。
...
**必问底线（问清楚比直接做重要 · 任何模式含自主/无人值守）**：新建项目/技术选型/地基决策（存储/架构/打包形态）前必问场景清单，一次结构化问全再动手——①使用：跨设备？分发形态（自用/打包 exe/交付他人）？②数据：规模增长/敏感度/备份与迁移预期？③环境：目标机 OS/运行时/浏览器/离线要求？④偏好：视觉风格与交互参照？⑤演进：功能预期/维护者/验收标准？⑥性能/安全/交付约束（验收/时间/技术栈偏好/依赖容忍）同样必问；问题宁多勿漏，全景清单见细则 #306；「你看着选」类笼统授权只覆盖明示项，未答项仍必问；决定须有依据（用户原话/调研档），禁凭空想象；环境盘点与成熟设计调研前置到计划前，不得以「诚实声明未调研」替代调研义务（见 §2.4 步 2/3）。**设计确认先于写码——设计档落档后暂停呈现用户确认再实现（细则 #312）；自主会话降级为落档+暂停行声明。**
...
| 新项目/地基决策（存储/打包/技术栈） | 暂停；先问场景清单（跨设备/分发/数据/环境/偏好/演进，细则 #306）再选型 |
...
**偏好记忆**：用户做出确认的选择（如经提问工具选定的技术栈 / 语言 / 风格）后，写入 agent-log 偏好段（§10）；**写入后主动向用户复核大类方向**（技术栈 / 语言 / 风格方向是否对头），用户指出偏离则按其修正；同类场景下次直接采用，不再重复询问。密钥与破坏性意图绝不写入偏好。
...
在任务记录中留一行仲裁记录（`来源 A vs 来源 B → 采用 X，因为 …`）。同一理由的冲突被裁决两次，即升格为常设决策：写入 agent-log 偏好段，不再散落临时笔记——**升格必带「前提 + 重开条件」两字段**（前提失效即可重开，details #279）。
...
目标模式附加职责：执行**前**写计划（范围、风险评级、时间/轮次预算）；子任务按**依赖边界**拆分（details #281：先依赖分析——强依赖保持串行不硬并行，独立子任务并行且带五要素，合并后统一集成验证）；边执行边记录；超预算自动停止；结束时交付复盘文档 + 待确认清单。**微轮次豁免**：cron 空转轮/无人值守微轮次（例行巡检/单轮无实质改动）允许压缩为「一轮一行+GATE」——标注「微轮次豁免」即声明式豁免，静默跳过仍违规。**每条用户消息（含任务中途消息）都要检查：①模式关键词 ②按 §0 复述前置（每轮首产物=复述：追加/继续=一行「收到 X｜理解为 Y｜边界 Z」；状态行在开工/阶段边界）**——触发词是活开关，会静默改变决策模型。；**降采样合法**：长会话/轻任务按比例降采样（阶段复述+块级一行档案+阶段级 GATE），显式声明即合规

### 5.2 判级速查（10 秒定论，一句话即止，禁止展开论证）

> **本块是 L3 封闭清单、L1/L2 速判与三条跑道分流的唯一权威源**；`rules.md`、`workflows.md` 等引用处只做摘要并回指本节。**三级同步链（三级必须一致）**：`injection-core.md` 因注入环境自包含必须保留判级要点全文（L3 清单/L1/L2 速判/三问分流——是其一例外），且它已被部署为平台全局注入副本——**本块 → `injection-core.md` → 已注入的平台全局副本**三级必须同步；改判级先改本块，再同步 injection-core，最后重新部署到注入点。
...
| 三问 | 命中 | 走哪条 |
|---|---|---|
| ① 跨 ≥3 包 / 跨 api+contracts+前端？② 涉契约·架构·迁移·对外发布·安全？③ 用户点名「按流程 / 严格分析」？ | **≥2 命中** → | **L2-F 完整 9 步**（§2.4） |
| | 其余 → | **L2-S 短工作流**（§2.3，默认） |

**自检**：□ 判级落在 GATE level= 字段（判级并入 GATE 输出，不设独立判级行）□ 命中 L3 → L2-F + 暂停行（先问；本地备份取代不了确认）□ L2-F 的边界模板已写（新端点+契约+两页 / 架构 / 迁移 / 密钥+发布 / 多模块联合）。

## 6. 最小闭环交付（L2-F 第 9 步的交付原则）
...
6. **有效性诚实声明**：规则有效性（对照 A/B）**未达显著**——本工作流**不宣称**「规则带来正确性优势」、不宣称泛化改进；纪律提供的是可追溯 / 可审计 / 防返工 / 点破后快速恢复机制（完整数据与实验设计见 EVIDENCE.md）。**触达是比内容更紧的瓶颈**：已安装 ≠ 被加载（主会话靠注入副本、子代理靠委托纪律包直送——见 §8/§12 AG）；「规则在场 ≠ 规则被遵守」。

## 7. 质量门禁与回滚

**触发**：提交前 / 破坏性变更前。

- **GATE 块（每任务块末尾；9 字段）**：字段语义与格式**唯一权威=注入核心「交付与留档」GATE 段**（本节不维护同步副本，防双份漂移）；分级形态：包级（任务块收尾）=完整 9 字段一行；子块/轻任务=行内简式（`GATE: {level=…, v=…, exit=…}` 三字段即可）；形态恒为单行（英文短键+中文值+等号大括号符号，定版标准），禁展开多行代码块。可重跑的工件 > 自我叙述；验收权在用户；`approval:never` 只豁免工具级审批，**永不豁免确认义务**。
...
- **流程不可跳步**：双调研（第 3 步）最容易被跳过——且三问分流后的「小模块免全量调研」不是跳步，是选道（§2.3 边界写明省什么）。
...
- **绝不覆盖已有规则文件**（`AGENTS.md`、`CLAUDE.md`、`.trae/rules/project_rules.md`、`.cursor/rules/*.mdc`、`.windsurferules` 等平台变体）：只备份 + 合并。
...
- **细则层「活着」不是装饰**：错误 / API 返回意外形态 / 未知字段或端点 / 新依赖不生效 → 先对 TOP 内联处置，**处置完成后留 errpath 行**（症状→处置路径，§9 错误处置入口），lookup=佐证资源非事前门槛；命中后在任务记录留一行引用；会话末报告命中数（0 照报 0）——只认真实 lookup 输出，不靠自述。
- **同坑 ≥3 次即晋升**：同一踩坑在单项目 ≥3 次（或高返工成本）→ 晋升进 agent-log 教训区 T1 TOP 列表（一行症状 + 一行对策）；2 个干净周期无命中 → 降级。
...
| 文件 | 内容 | 何时加载 / 触发症状 |
|---|---|---|
| `references/injection-core.md` | 平台无关核心（常驻瘦身版：判级速查/开工四步/三模式/红线/GATE 9 字段/TOP+检索端口/状态行常驻，流程细节回指本文件；每会话注入在场） | 第 3 节强制注入（硬加载）时；「写进我的平台规则」 |
...
| `references/workflows.md` | 总纲与澄清流程（§0.0-0.1）/ 联网调研可信依据（§0.2）/ 完善度诊断（§0.3）/ 双调研规划（§0.4）/ 调研矩阵与分流决策表（§0.5-0.6）/ 9 类任务工作流（§1-§9）/ 复用五问 / 质量门禁细节 | 现状模糊 / 任务类型不明 / 规划质量存疑 / 产品完善度诊断 |
| `references/details.md` | 落地细则（工程规范 / 具体做法）——细则 344 条·25 类（类数=分节数，facts_sync 断言），症状索引 | 症状关键词命中踩坑类别（[契约] / [构建] / [前端] / [运维] …真实标签集，见索引表）——**在不熟悉领域即兴发挥前，先查这里** |
...
| `templates/agent-log-template.md` | 一档制档案骨架（§10）——初始化状态段/教训区/偏好段/流水区四区 | 项目尚无 `memory/agent-log.md` |
| `templates/project-rules.md` | 项目级注入规则文件模板（在场锚点 + 项目纪律 + 承载清单）——「项目承载」第五层 | §3 第 6 步项目承载检查③；§2.0 开工四步第 2 步；项目工作区无项目级规则文件（按 `platform-adaptation.md` 项目级注入点表定名） |
...
**错误处置入口（先对 TOP 内联处置，事后留 errpath）**：遇到错误 / API 意外形态 / 未知字段或端点 / 新依赖不生效 → **先逐条对照「错误必查 TOP」内联处置**（下）；需要按症状类搜 `details.md` 时（如 [契约] / [构建] / [测试] 等真实标签，见头部症状索引）或按关键词检索 → `python "<技能安装目录>/scripts/detail_lookup.py" "<症状关键词>"` 一条命令完成检索（技能安装目录=平台解析到的 Base directory）——**lookup=佐证资源非事前门槛**；**处置完成后必留 errpath 行**：症状→处置路径（TOP 命中 / lookup 执行[贴命中行] / 未跑[理由]）；影响决策时任务记录留一行引用——**统一写完整前缀形态 `details #N 命中：…` / `细则 #N`，禁裸 `#N`**（裸编号无法与 GitHub issue 编号区分，假阳性实证 9/10）。

**错误必查 TOP**（唯一权威源 = injection-core 错误段，本处为回指同步——错误发生时先对这几条内联处置）：#233 凭命名直觉写对接=假绿（先 grep 调用点 → 读 schema → 确认包 → 再写）｜#228 改共享包先重编（旧 dist 是类型真相）｜#229 常驻进程旧 dist（行为不符先查 uptime/时间戳，判别先行）｜#214 响应形态分层断言（成功=裸数据 / 校验失败=2xx 信封 / 真 404=真状态码）｜#163 统一错误契约（code≠0 才算失败，data:null 是合法成功）｜#256 / #269 异步栈丢调用点 / 响应体只消费一次｜#262 深拷贝/序列化语义变体（undefined 键丢弃）。**淘汰机制**：TOP 条目挂最近命中证据，2 个干净周期零命中降级（同 experience-mustread 降级制；命中证据记录于 EVIDENCE）。
...
- **会话始**：执行 **§2.0 开工序列四步**（复述理解 → 承载检查 → 记忆对齐 → 判级选道——权威源在 §2.0，此处不重复条款；承载三件=memory 一档（`memory/agent-log.md`）/docs 六节/项目级规则文件按平台注入点表定名）。**记忆对齐最小读取**：读 `memory/agent-log.md` 状态段一屏；教训区 T1/T2、偏好段、流水区**按当前症状精确检索命中条目，不整篇载入**；平台有原生记忆机制（auto-memory / 记忆文件）时不重复预读——跨会话偏好与事实续接交给平台记忆，本档聚焦项目审计。**记忆路由裁决**：平台原生记忆与项目「开工必读」条款冲突时项目优先（仲裁序），并在项目规则文件/agent-log 首行落一行「本项目权威承载=X」（防双源漂移）。上下文预算法（先定序，防污染）：**常驻**（本核心，小）→ **开工读**（agent-log 状态段 + 按症状检索，最小读取）→ **按需**（`references/`、流水区与轮转档历史，到步骤才读，不预载全部引用）→ **结束更新**（最小追加，见「完成后更新序」）。
- **块尾**：流水区追加一行（改动 → 验证(结果) → 未验证项 → GATE）——时间戳 **`YYYY-MM-DD HH:mm:ss`**（秒级必填；日级 = 不完整）；**活头部校验**：头部 `更新：` 时间戳必须 ≥ 最新正文条目时间；**容量上限**：流水区 >200 行或教训区 >150 行 → 旧行/旧条移 `agent-log-archive-YYYY-MM.md`（移动非删除）。
- **会话末**：①最小验证 + 自查（真实可用 / 边界 / 规则 / 文档同步）→ **交付面细则 #327-#331（GATE refs 实测/系统级配置判级/增量解释显式化/接力链对账/纯文档会话 commit）随交付动作按需查 details** → ②更新 `memory/agent-log.md`（流水区追加本块一行 + 状态段刷新，结论即时落盘）+ **每项重要决策一行决策审计**（决定 / 依据【含前提 + 重开条件，details #278】/ 被否候选 / 影响，落流水区）→ ③更新教训区（新踩坑或重复坑：症状→根因→解决→预防；重复内容只写一处并交叉引用）→ ④更新偏好段（已确认偏好 + 复核提醒；密钥与破坏性意图绝不写入）→ ⑤文档与代码同批提交；会话结束提炼 1-5 条可复用知识点（默认 3 条）。所有关键回滚走**本地备份优先**、push 仅在需远程保护 / 交付时。
...
- **工作区 `memory/` 一档制 + 自动创建**：项目根 `memory/`——跨会话状态 / 踩坑 / 偏好 / 任务流水统一归档于**单文件 `agent-log.md`**（四区：状态段 / 教训区 / 偏好段 / 流水区）；**任何会话（含下一个 AI）开工先读本档**，不存在则从 `templates/agent-log-template.md` 复制创建。业务真实恰用 `memory/` 时，可在项目规则文件内把归档目录改为 `.agent-records/`（唯一合法覆盖点）。旧五件套分散文件项目已就绪沿用，增量写入本档。`memory` = 状态层 + 踩坑层，完整规则仍在 `references/` 按需加载——互不替代。
- **状态段是骨架不是日记——硬上限 ≤10 行**。只放当前阶段 / 下一步 / 遗留 / 红线 / 回滚基线。开工扫描发现超限时：**先把里程碑史迁移到 `agent-log-archive-YYYY-MM.md` 再开工**（移动而非删除）。
- **经验回流（双击晋升制）**：项目级踩坑先落 agent-log.md 教训区 T2。同一踩坑**在单项目出现两次、或跨两个项目各出现一次**（症状相符、根因相同）即触发晋升：按「症状→根因→解决→预防」提炼为可归类条目，提交进**本 Skill 的 `references/details.md`**——让后续所有项目的第 2 步症状检索都能命中。回流是「已稳定教训的晋升」，不是首例写入；一次性噪音永不进 Skill。
- **用户偏好**：用户做出确认的选择（技术栈 / 语言 / 风格）后写入 agent-log.md 偏好段；写入后主动复核大类方向；会话开始时读取；同类决策直接采用。**语言表达默认中文**（回答 / 思考 / 文档 / 注释；代码标识符、标准术语除外）——偏好段记录了偏好语言时以记录为准，未记录时默认中文，不跟随会话输入语言。
- **配套模板**：规划、验收标准、任务记录、复盘、回滚点、提示词预算、一档制档案模板（agent-log-template），以及会话钩子（`templates/hooks/`）与审查子代理（`templates/agents/`）均位于 `templates/`（复制后填写，不原地编辑）。
...
| id | 触发 | 动作 | 模板 / 自检 | 边界 |
|---|---|---|---|---|
| G1 | 任务块结束 | 写 GATE 行（9 字段） | `GATE: {level,v,cmd,exit,files,refs,errpath,lessons,exempt}`；cmd 可重跑；权威定义=注入核心 GATE 段 | errpath=症状→处置路径（事后化，无错误填—）；判级并入 level |
| S1 | 会话结束 | 状态面（§11） | 三行起 | 不是通过 / 失败声明 |
| A1 | L2/L3 计划 | 产品五问（§2.4 步 6） | 诉求分解 / 被否候选 ≥1 / 返工成本 / 边界 / 验收 3-5 | L3 增加风险层证据 |
...
| P2 | 同坑 ≥3 | 晋升 agent-log 教训区 T1；2 个干净周期降级 | 一行症状 + 一行对策 | 只写一处（交叉引用） |
...
| P8 | 上下文 40-60%（紧凑档 25-35% 窗口）/ 压缩信号 | **折叠协议（details #272）**：①保留清单五必留核对（任务本质/验收/激活规则原文/当前步骤/回滚基线）②checkpoint 落盘（agent-log 状态段+流水区）③旧块一行摘要入归档 ④重载序（§9 守卫）⑤状态面记「已折叠 N 块」 | 五必留不全 → 先补再折叠 | 激活规则原文原样保留，**不折叠** |
...
| R1 | 计划被拒 / 用户改向 | 拒绝日志：原话 + 隐含需求 → 偏好（agent-log 偏好段）或需求变更（task record）→ 回流 | 各一行 | 拒绝是金矿，不是噪音；**拒绝记录同带前提，重开合法**（details #279） |
...
| AG | 委托子代理 | **必须把最小纪律包内联进子代理 prompt（分级：只读=一行包；实施/写路径=全包，判据=写路径非任务大小）**（判级 / 红线 / 证据含不复现四要件 / 错误必查 TOP / 引用形态 / GATE / **承载**：子代理独立工作区先建一档制 agent-log.md（四区）+ 项目级规则文件摘要——不指望子代理自加载 Skill） | rules §28 | 子代理不继承注入副本、触发不可靠（实证：提醒可读仍 0 加载）；批派≤3/批防限流（#281⑤） |

## 13. 模板索引

可复制即用的模板在 `templates/`：规划 / 验收标准 / 任务记录（含 GATE 字段）/ 复盘 / 回滚点 / 提示词预算 / 一档制档案模板（agent-log-template）/ 会话钩子（hooks）/ 审查子代理（agents：critic / risk-reviewer / security-auditor）。复制后填写，绝不原地编辑。
