...
| 4 | **判级选道**：§5.2 判级速查 + 三问分流 → 三级跑道 | 选道一行 |
...
| 新项目/地基决策（存储/打包/技术栈） | 暂停；先问场景清单（跨设备/分发/数据/环境/偏好/演进，#306）再选型 |
...
- **GATE 块（每任务块末尾，第 11 步出口产物；9 字段，权威定义=注入核心「交付与留档」GATE 段，本处为同步副本）**：一行 `GATE: {level=<L1/L2-S/L2-F>, v=<范围>, cmd=<可重跑命令>, exit=<退出码>, files=<变更文件>, refs=<细则引用计数(grep 自查,0 照报)>, errpath=<症状→处置路径,无错误填—>, lessons=<知识点>, exempt=<未验证声明>}`。**可重跑的工件 > 自我叙述**；验收权在用户；`approval:never` 只豁免工具级审批，**永不豁免确认义务**。**level 字段**：判级并入 GATE 输出（§5.2 判级速查仍要做），不设独立判级行。**refs 字段**：会话末 grep 细则引用行计数（0 照报 0）。**errpath 字段（事后化）**：遇错误先对「错误必查 TOP」内联处置，处置完成后填 `症状→处置路径`（TOP 命中 / lookup 执行[贴命中行] / 未跑[理由]）——lookup=佐证资源非事前门槛，**以 lookup 命令输出为证据，不接受自报**（防自报污染；域见 details 头部症状索引）；无错误任务填 `—`。**分级形态**：包级（任务块收尾）=完整 9 字段一行；子块/轻任务=行内简式（level/v/exit 三字段即可）；形态恒为单行（英文短键+中文值+等号大括号符号，定版标准），禁展开多行代码块。
...
- **会话末**：①最小验证 + 自查（真实可用 / 边界 / 规则 / 文档同步）→ ②更新 `memory/agent-log.md`（流水区追加本块一行 + 状态段刷新，结论即时落盘）+ **每项重要决策一行决策审计**（决定 / 依据【含前提 + 重开条件，details #278】/ 被否候选 / 影响，落流水区）→ ③更新教训区（新踩坑或重复坑：症状→根因→解决→预防；重复内容只写一处并交叉引用）→ ④更新偏好段（已确认偏好 + 复核提醒；密钥与破坏性意图绝不写入）→ ⑤文档与代码同批提交；会话结束提炼 1-5 条可复用知识点（默认 3 条）。所有关键回滚走**本地备份优先**、push 仅在需远程保护 / 交付时。
