...
- **复述前置（首产物状态行 + 阶段边界复述）**：开工先严谨复述目标/边界+规则要点再动手——不自以为是「理解了就理解了」；并输出一行可 grep 状态行 `Context: state=<读档/新建/单发> L=<L1/L2-S/L2-F> confirm=<无需/已问/豁免:理由>`（判级并入 L= 字段）。理解缺口 → **先补依据**（澄清提问或现场查证）再继续。任务中途消息**不逐条全量复述**（逐消息复述=悬空义务，实测触发趋零）——改为**阶段边界复述**：开工 / 收尾 / L3 决策点 / 方向变化时显式复述确认；**L3 确认不豁免**；摘要接续的会话可省开工复述，但须一行声明「接续跳过复述，依据=摘要上下文完整」（跳过+声明合法）；L1/安静模式复述并入状态行，不另展开（防 token 浪费）。
...
目标模式附加职责：执行**前**写计划（范围、风险评级、时间/轮次预算）；子任务按**依赖边界**拆分（details #281：先依赖分析——强依赖保持串行不硬并行，独立子任务并行且带五要素，合并后统一集成验证）；边执行边记录；超预算自动停止；结束时交付复盘文档 + 待确认清单。**微轮次豁免**：cron 空转轮/无人值守微轮次（例行巡检/单轮无实质改动）允许压缩为「一轮一行+GATE」——标注「微轮次豁免」即声明式豁免，静默跳过仍违规。**每条用户消息（含任务中途消息）都要检查：①模式关键词 ②按 §0 复述前置（首产物状态行+阶段边界复述）**——触发词是活开关，会静默改变决策模型。；**降采样合法**：长会话/轻任务按比例降采样（阶段复述+块级一行档案+阶段级 GATE），显式声明即合规
