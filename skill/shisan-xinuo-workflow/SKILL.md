...
**必问底线（问清楚比直接做重要 · 任何模式含自主/无人值守）**：新建项目/技术选型/地基决策前，场景清单一次结构化问全再动手——①使用：跨设备？分发形态（自用/打包 exe/交付他人）？②数据：规模/敏感度/备份与迁移预期？③环境：目标机 OS/运行时/浏览器/离线？④偏好：视觉风格与交互参照？⑤演进：功能预期/维护者/验收标准？「你看着选」类笼统授权只覆盖明示项，未答项仍必问；决定须有依据，禁凭空想象（细则 #306-#310；环境盘点与调研前置，见 §2.2 步 2）。
