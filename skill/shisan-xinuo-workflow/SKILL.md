...
6. **有效性诚实声明**：本工作流「规则改变行为」的有效性证据 = 2026-08-30 一轮 4 任务 A/B 迷你实验（四项指标无差异，**未达显著**）——机制按既定拍板保留，但不得宣称已证明有效；更大样本 / 更长周期复验列后续轮次。
...
| `references/details.md` | 落地细则（工程规范 / 具体做法）——14 类，症状索引 | 症状关键词命中踩坑类别（构建工具链 / 框架版本 / 无障碍 / API 形态 / 部署 …）——**在不熟悉领域即兴发挥前，先查这里** |
