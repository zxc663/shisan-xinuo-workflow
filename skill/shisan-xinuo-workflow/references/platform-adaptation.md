...
| 平台 | 记忆层承载点 | 记忆内容 | 实测状态（2026-08-31 本机） |
|---|---|---|---|
...
**硬注入 = 三层强制**（用户拍板 2026-08-31）：记忆层 + 规则层 + 配置文件层**同时写入**，缺一不可；写前必须**提醒用户授权**（平台 / 三层目标 / 内容长度 / 每会话成本 / 影响范围）。**按需注入只写「应用层」**（项目 .trae/rules/project_rules.md 等，精简纪律约 9 行 + 询问用户是否写规则层；**不写记忆层**）。
