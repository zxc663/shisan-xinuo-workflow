...
>
> **引用规范（v2.0.4）**：任务记录 / 会话产物 / 状态面取证内引用本文件细则时**统一写 `details #NNN`（或 `细则 #NNN`）**，勿省略 `details` 前缀或写成其它变体——状态面取证命令按此形态计数（`grep -cE 'references/details|details[ #]+[0-9]{2,3}|细则[ #]*[0-9]{2,3}|#[0-9]{3}'`，含行内裸编号引用兼容）。本文件条目本身是 `1.–254.` 有序列表（无 `#` 前缀），不是取证对象。
