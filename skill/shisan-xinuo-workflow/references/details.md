...
>
> **引用规范**：任务记录 / 会话产物 / 状态面取证内引用本文件细则时**统一写完整前缀形态 `details #NNN`（或 `细则 #NNN`）**——**禁止裸 `#NNN`**：裸编号无法与 GitHub issue 编号区分（假阳性实证 9/10），状态面取证命令只认前缀形态（`grep -cE 'references/details|details[ #]+[0-9]{2,3}|细则[ #]+[0-9]{2,3}'`）。本文件条目本身是 `1.–305.` 有序列表（无 `#` 前缀），不是取证对象。
>
> **维护规则（防棘轮）**：新增细则前先 grep 比对既有条目，语义重复**当场合并**（保留旧编号，新场景并入该条正文）；删减与降级（移层 / 标注废弃）合法且鼓励——本文件以「语义不重复、检索面收敛」为维护目标，不以条数增长为成果。**特殊槽**：#295=预留槽（候选等双击激活，占号非细则）；#180=归档行（已并入 #232）——条数口径以活跃条目计（deploy/verify 排除特殊槽），309 条·17 类。
>
...
## 21. 真实项目路测回流（2026-09-12 · personal-workbench 长会话混合指令真测，条目 306-311）

> 路测 v6 首轮（本地个人工作台真实项目，A1+5 变体指令），用户旁观逐条点破——「问清楚比直接做重要」升格纪律。

306. [流程] **技术选型前场景清单必问（问清楚比直接做重要）**：新建项目/技术选型/地基决策前，场景清单一次结构化问全再动手——①使用：跨设备？分发形态（自用/打包 exe/交付他人）？使用频率？②数据：规模与增长？敏感度与备份预期？既有数据迁移？③环境：目标机 OS/运行时/浏览器/离线要求？④偏好：视觉风格与交互参照？⑤演进：功能预期/维护者/验收标准？「你看着选」类笼统授权只覆盖明示项，未答项仍必问；决定须有依据（用户原话/调研档），禁凭空想象。*来源/晋升*：路测 v6 A1 实证（自决 localStorage+主题配色+全程未问跨设备/分发/风格，用户点破「万一在另一台电脑用？万一主题不喜欢？」，2026-09-12）；检索键：必问/需求。
307. [运维] **git init + 阶段性 commit = 项目首日义务**：新项目开工首日先 `git init`+初始 commit，此后每任务块收尾一 commit（回滚点粒度=任务块）；「重大改动前建回滚点」红线的前置是版本控制在场——全程无 git 时重构/删文件/改架构均无回退，手工备份目录不是替代品。*来源/晋升*：路测 v6 实证（全会话无版本控制，V4 才建 legacy-server/ 手工回滚点，2026-09-12）；检索键：git/回滚。
