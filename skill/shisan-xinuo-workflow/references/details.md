...
>
> **引用规范（v2.0.5）**：任务记录 / 会话产物 / 状态面取证内引用本文件细则时**统一写完整前缀形态 `details #NNN`（或 `细则 #NNN`）**——**禁止裸 `#NNN`**：裸编号无法与 GitHub issue 编号区分，2026-08-31 路测实证 v2.0.4 取证命令 10 计数中 9 为 issue 编号误计（假阳性），v2.0.5 起状态面取证命令只认前缀形态（`grep -cE 'references/details|details[ #]+[0-9]{2,3}|细则[ #]+[0-9]{2,3}'`）。本文件条目本身是 `1.–275.` 有序列表（无 `#` 前缀），不是取证对象。
...
## 17. 上下文主动管理补全（2026-09-01 · 借鉴 Aider repo-map / 1bcoder /map·/ctx·/scan / Atrium Preserver 机制回流，条目 272-275）
