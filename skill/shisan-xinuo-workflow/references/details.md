...
>
> **维护规则（防棘轮）**：新增细则前先 grep 比对既有条目，语义重复**当场合并**（保留旧编号，新场景并入该条正文）；删减与降级（移层 / 标注废弃）合法且鼓励——本文件以「语义不重复、检索面收敛」为维护目标，不以条数增长为成果。**特殊槽**：#295=预留槽（候选等双击激活，占号非细则）；#180=归档行（已并入 #232）——条数口径以活跃条目计（deploy/verify 排除特殊槽），314 条·17 类。
>
