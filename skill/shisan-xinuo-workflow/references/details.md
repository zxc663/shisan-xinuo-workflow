...
>
> **维护规则（防棘轮）**：新增细则前先 grep 比对既有条目，语义重复**当场合并**（保留旧编号，新场景并入该条正文）；删减与降级（移层 / 标注废弃）合法且鼓励——本文件以「语义不重复、检索面收敛」为维护目标，不以条数增长为成果。**特殊槽**：#180=归档行（已并入 #232；#295 已于 2026-09-16 双击转正）——条数口径以活跃条目计（deploy/verify 排除特殊槽）；活跃 343 条，类数=分节数 24（**单一权威源=本文件分节计数**，`scripts/facts_sync.py` 断言全仓口径与分节数一致）。
>
