...
```
grep -cE 'references/details|details[ #]+[0-9]{2,3}|细则[ #]*[0-9]{2,3}|#[0-9]{3}' <会话产物 / 任务记录>   # 统计细则命中数；0 照报 0
```

> 取证对象 = **会话产物 / 任务记录中的细则引用**（`details #239`、`细则 #233`、行内裸 `#228×3`），**不是 details.md 自身条目**——其条目是 `1.–254.` 有序列表（无 `#` 前缀）。旧命令 `#[0-9]{2,3}\.`（要求井号+句点）对两种真实形态都打不中、恒 0 假阴性（v1.13 起即存在，v2.0.4 修复）；引用细则时统一写 `details #NNN`（见 details.md 头部引用规范）。
