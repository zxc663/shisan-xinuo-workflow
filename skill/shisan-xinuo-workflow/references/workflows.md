...
> 取证对象 = **会话产物 / 任务记录中的细则引用**（`details #239`、`细则 #233`），**不是 details.md 自身条目**——其条目是 `1.–283.` 有序列表（无 `#` 前缀）。**只认完整前缀形态**——裸 `#NNN` 分支已废弃（无法区分细则编号与 GitHub issue 编号，假阳性实证 9/10）；引用细则统一写 `details #NNN`（见 details.md 头部引用规范）。
