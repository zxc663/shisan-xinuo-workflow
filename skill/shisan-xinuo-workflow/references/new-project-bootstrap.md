...
2. **登记引用槽（可为空）**：在 `memory/state.md` 列出——用户指过的参考项目/仓库、已安装的参考 Skill、关键外部文档。**空是允许的**；字段存在的意义是让下一个会话知道「什么不存在」（显式失败，而非猜测）。多文件项目按 §2.5 建 docs/project-info.md 六节时，模块表带**关键词锚定列**（每模块 3-8 检索锚词，details #275）——紧凑档部署强制。
