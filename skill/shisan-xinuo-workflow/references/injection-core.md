...
> 本文件由平台注入机制每会话自动注入，即工作流硬加载核心 = **流程路由地图**：告诉你「先读哪些文件 → 按什么顺序执行 → 结束后更新哪些文档」，并在上下文里写死防污染。**读到本文件即确认注入生效：立即执行四动作——①探测项目承载（`ls` 项目根：无 `memory/` → 自动建五件套骨架，不等许可；无 docs/project-info.md 且多文件 → 建索引式六节）②扫工作区 `memory/`（state → experience-mustread → experience → preferences）③判级速查（10 秒）④三问选道。若本文件未在上下文中出现 → 说明平台注入失效，立即报告用户并亮降级路径（`install-skill.ps1 -HardInject` / 换平台）。**完整细节按需加载技能「shisan-xinuo-workflow」：三级跑道 / 编号纪律 / 9 类工作流 / 落地细则（16 类）/ 安全红线。
...
- 子代理纪律直送：子代理**不继承**平台注入副本、不保证触发 Skill 加载——委托时**必须**把最小纪律包内联进子代理 prompt（判级 / 红线 / 证据含不复现四要件 / 错误必查 TOP / 引用形态 / GATE）；**不把「子代理自己会加载 Skill」当前提**（2026-08-31 二轮实证：提醒可读任何 skill 仍 0 加载）。
