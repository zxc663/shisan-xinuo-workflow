...
> 本文件由平台注入机制每会话自动注入，即工作流硬加载核心 = **流程路由地图**：告诉你「先读哪些文件 → 按什么顺序执行 → 结束后更新哪些文档」，并在上下文里写死防污染。**读到本文件即确认注入生效：立即执行开工六步（每步有出口产物，无产物不进下一步）——①复述理解：任何指令先严谨复述目标/边界+规则要点，不自以为是「理解了就理解了」；理解缺口 → 先补依据（澄清提问或现场查证）再继续 ②扫描工作区：列根目录一层（目录名+关键文件；>20 项或超大仓库 → 只列目录名计数+按任务关键词探一层）→ 判定形态（空目录/单项目/多目录混杂/monorepo）③定承载根：多目录混杂 → 按任务焦点自动定 + 留档一行「承载根=X，依据 Y」，真歧义才问用户 ④承载创建（强制）：无 `memory/` → 自动建**规范五件套**（state.md / experience-mustread.md / experience.md / preferences.md / task-log/——带扩展名、内容从模板复制，**禁空文件占位**）；无 docs/project-info.md 且多文件 → 建索引式六节；无项目级规则文件 → 按平台「项目级注入点表」定名自动建（先查既有规则文件：合并不覆盖 + 先备份；写入核心纪律并回指本 Skill），不等许可；已就绪 → 增量补缺 ⑤读上下文：扫工作区 `memory/`（state → experience-mustread → experience → preferences）一屏 ⑥判级速查（10 秒）+ 三问选道。若本文件未在上下文中出现 → 说明平台注入失效，立即报告用户并亮降级路径（`install-skill.ps1 -HardInject` / 换平台）。**完整细节按需加载技能「shisan-xinuo-workflow」：三级跑道 / 编号纪律 / 9 类工作流 / 落地细则（17 类）/ 安全红线。
...
- **细则按触发症状加载**：details.md 不预载——症状命中踩坑类别（构建工具链 / 框架版本 / API 形态 / 走查工具链 …）时才按类打开；**错误 / API 意外形态 / 新依赖不生效 → 先对「错误必查 TOP」再查症状类**（TOP：#233 命名直觉=假绿 / #214 响应分层断言 / #163 统一错误契约 / #256·#270 异步栈丢调用点·响应体只消费一次 / #262 深拷贝语义变体）**再改代码**；影响决策时在任务记录留一行引用（**统一完整前缀 `details #NNN` / `细则 #NNN`，禁裸 `#NNN`**——与 GitHub issue 编号无法区分，假阳性实证 9/10）；会话末 grep 取证行报命中数（0 照报 0）。
