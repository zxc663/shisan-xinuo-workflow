...
- **结束更新（最小追加）**：见「完成后更新序」。
- **开工一行自检（每会话，前缀触达）**：能确定本 Skill 安装目录名（Base directory，来自平台系统提示 / 技能列表路径 / 用户告知）时核对 `agent-` 前缀——无前缀 → 一行提示「`scripts/install-skill.ps1` 可一键带前缀重装；不愿改则 state.md 记『保持无前缀』后此后静默」；目录名未知 → 不猜测、不阻塞任务，状态面记一行（权威源：SKILL §3 第 0 步）。
...
- 负向结论更严格：判「不复现 / 不存在」举证责任高于「复现」——①判据与对象逐字对齐 ②实验复现真实调用链（真实构建产物，**禁自造模拟代替真实代码**）③有对照组；不满足任一 → 降「未定论」，不判不复现（四要件权威源：SKILL §8；细则：details #255）。
- 改动验证覆盖 ≥2 种负载形状（快 / 慢任务交错），单负载下 `Promise.race` 类方案缺陷不可见（2026-08-31 路测 B 轨：规则内唯一被认定有实际价值的条；细则：details #266）。
- 子代理纪律直送：子代理**不继承**平台注入副本、不保证触发 Skill 加载——委托时**必须**把最小纪律包内联进子代理 prompt（判级 / 红线 / 证据含不复现四要件 / 错误必查 TOP / 引用形态 / GATE / **承载**：子代理独立工作区先建规范 memory 五件套 + 项目级规则文件【templates/project-rules.md】摘要——**不以空文件占位**）；**不把「子代理自己会加载 Skill」当前提**（2026-08-31 二轮实证：提醒可读任何 skill 仍 0 加载）。
- 项目内写文件 = 自动动作：工作区 `memory/` 骨架、docs/、代码 = 直接创建 / 修改，**不触发授权暂停**（2026-08-31 实测：把「自动建 memory」误当「需授权」→ 承载漏建）；授权仅限：平台全局注入文件、密钥/权限、数据删除/迁移、对外发布、超预算破坏性。
...
- **细则按触发症状加载**：details.md 不预载——症状命中踩坑类别（构建工具链 / 框架版本 / API 形态 / 走查工具链 …）时才按类打开；**错误 / API 意外形态 / 新依赖不生效 → 先对「错误必查 TOP」再查症状类**（TOP：#233 命名直觉=假绿 / #214 响应分层断言 / #163 统一错误契约 / #256·#270 异步栈丢调用点·响应体只消费一次 / #262 深拷贝语义变体）**再改代码**；影响决策时在任务记录留一行引用（**统一完整前缀 `details #NNN` / `细则 #NNN`，禁裸 `#NNN`**——与 GitHub issue 编号无法区分，2026-08-31 路测实证假阳性 9/10）；会话末 grep 取证行报命中数（0 照报 0）。
...
- **保留清单+折叠协议（Preserver，v2.1.0 借鉴 Atrium）**：压缩/折叠/新会话交接前核对五必留——①当前任务本质一句话 ②验收标准 3-5 条 ③激活规则原文（注入核心+已加载引用，原样保留不折叠）④当前步骤与下一步 ⑤回滚基线；不全 → 先补齐再压缩。折叠触发 → checkpoint 落盘（state.md+task-log）→ 旧块一行摘要入归档 → 重载序（SKILL §9）。细则：details #272。
- **紧凑档（短上下文部署，v2.1.0）**：本地小模型/短窗口（≤32K）→ `memory/preferences.md` 标注「紧凑档」→ 折叠阈值按窗口比例 25-35%（替代标准档绝对阈值）、prompt-budget 强制 ≤minimal、模块锚点表+大文件读取协议强制、状态面一行式；**纪律不降级**——注入核心常驻全文保留（Preserver 原则：规则原文是保留清单必留项，砍纪律=自造失忆幻觉）。细则：details #273/#274/#275。
