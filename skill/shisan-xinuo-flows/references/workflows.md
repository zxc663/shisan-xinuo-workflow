...
每个任务块末尾一行 **GATE 块（11 字段）**——**权威定义见注入核心「交付与留档」GATE 段**，字段清单：`level / v / cmd / exit / files / refs / errpath / lessons / exempt / caps / effort`（判级并入 level 字段，不设独立判级行；errpath 事后化=症状→处置路径，无错误填 `—`）。可重跑工件 > 自我叙述；验收权在用户；`approval:never` 只豁免工具级审批，**永不豁免确认义务**。任务记录模板带有此字段。
